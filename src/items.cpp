...
int32_t Items::loadFromOtb(std::string file)
{
    ...
    {
        std::cout << "[Error - Items::loadFromOtb] Old version detected, a newer version of items.otb is required." <<
            std::endl;
        return ERROR_INVALID_FORMAT;
    }
    else if (Items::dwMajorVersion > 3)
    {
        std::cout << "[Error - Items::loadFromOtb] New version detected, an older version of items.otb is required." <<
            std::endl;
        return ERROR_INVALID_FORMAT;
    }
...
bool Items::loadFromXml()
{
    ...
    for (xmlNodePtr itemNode = itemRoot->children; itemNode; itemNode = itemNode->next)
    {
        ...
        else if (readXMLString(itemNode, "fromid", strValue) && readXMLString(itemNode, "toid", endValue))
        {
            ...
            if (intVector[0] && endVector[0] && intVector.size() == endVector.size())
            {
                ...
                std::cout << "[Warning - Items::loadFromXml] Malformed entry (from: \"" << strValue << "\", to: \"" <<
                    endValue << "\")" << std::endl;
        }
        else
    ...
    for (uint32_t i = 0; i < Item::items.size(); ++i) //lets do some checks...
    {
        ...
        //check bed items
        if ((it->transformToFree || it->transformUseTo[PLAYERSEX_FEMALE] || it->transformUseTo[PLAYERSEX_MALE]) && it->
            type != ITEM_TYPE_BED)
    ...
    for (xmlNodePtr paletteNode = paletteRoot->children; paletteNode; paletteNode = paletteNode->next)
    {
        ...
        {
            if (readXMLInteger(paletteNode, "chance", intValue) || readXMLInteger(
                paletteNode, "defaultChance", intValue))
            {
                if (intValue > 100)
                {
                    intValue = 100;
                    std::cout << "[Warning - Items::loadFromXml] Randomize chance cannot be higher than 100." <<
                        std::endl;
                }
            ...
            if (readXMLString(paletteNode, "randomize", strValue))
            {
                ...
                if (itemList.size() >= 2)
                {
                    if (itemList[0] < itemList[1])
                    {
                        ...
                        std::cout << "[Warning - Items::loadFromXml] Randomize min cannot be higher than max." <<
                            std::endl;
                }
                ...
                if (readXMLInteger(paletteNode, "chance", intValue))
                {
                    ...
                    {
                        intValue = 100;
                        std::cout << "[Warning: Items::loadRandomization] Randomize chance cannot be higher than 100."
                            << std::endl;
                    }
    ...
    while (itemAttributesNode)
    {
        if (readXMLString(itemAttributesNode, "key", strValue))
        {
            ...
            if (parseAttribute != ItemParseAttributesMap.end())
            {
                ...
                case ITEM_PARSE_FLOORCHANGE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            {
                                std::cout << "[Warning - Items::loadFromXml] Unknown floorChange: " << tmpStrValue <<
                                    std::endl;
                            }
                        }
                        break;
                ...
                case ITEM_PARSE_CORPSETYPE:
                    {
                        ...
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (tmpStrValue == "venom")
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown corpseType " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_FLUIDSOURCE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (fluid != FLUID_NONE)
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown fluidSource " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_WORTH:
                    {
                        if (readXMLInteger(itemAttributesNode, "value", intValue))
                        {
                            if (moneyMap.find(intValue) != moneyMap.end() && (!readXMLString(
                                itemNode, "override", strValue) || !booleanString(strValue)))
                                std::cout << "[Warning - Items::loadFromXml] Duplicated money item " << id <<
                                    " with worth " << intValue << "!" << std::endl;
                            else
                            {
                ...
                case ITEM_PARSE_WEAPONTYPE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (tmpStrValue == "sword")
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown weaponType " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_SLOTTYPE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            else if (tmpStrValue == "ring")
                            {
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown slotType " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_AMMOTYPE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (it.ammoType == AMMO_NONE)
                                std::cout << "[Warning - Items::loadFromXml] Unknown ammoType " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_SHOOTTYPE:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (shoot != SHOOT_EFFECT_UNKNOWN)
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown shootType " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_AMMOACTION:
                    {
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            if (ammo != AMMOACTION_NONE)
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown ammoAction " << strValue <<
                                    std::endl;
                        }
                        break;
                    }
                ...
                case ITEM_PARSE_FIELD:
                    {
                        ...
                        if (readXMLString(itemAttributesNode, "value", strValue))
                        {
                            ...
                            else if (tmpStrValue == "physical")
                            {
                                ...
                                std::cout << "[Warning - Items::loadFromXml] Unknown field value " << strValue <<
                                    std::endl;
                                        ...
                                        if (tmpStrValue == "damage")
                                        {
                                            if (readXMLInteger(fieldAttributesNode, "value", intValue))
                                            {
                                                ...
                                                if (start > 0)
                                                {
                                                    ...
                                                    for (std::list<int32_t>::iterator it = damageList.begin(); it !=
                                                         damageList.end(); ++it)
                ...
                default:
                    {
                        std::cout << "[Warning - Items::loadFromXml] Not configured key value: " << strValue <<
                            std::endl;
                        break;
                    }
                }
