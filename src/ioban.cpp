...
bool IOBan::isPlayerBanished(uint32_t playerId, PlayerBan_t type) const
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `bans` WHERE `type` = " << BAN_PLAYER << " AND `value` = " << playerId <<
        " AND `param` = " << type << " AND `active` = 1 LIMIT 1";
    if(!(result = db->storeQuery(query.str())))
        return false;
...
bool IOBan::addIpBanishment(uint32_t ip, int64_t banTime, uint32_t reasonId,
                            std::string comment, uint32_t gamemaster, uint32_t mask/* = 0xFFFFFFFF*/,
                            std::string statement/* = ""*/) const
{
    ...
    query <<
        "INSERT INTO `bans` (`id`, `type`, `value`, `param`, `expires`, `added`, `admin_id`, `comment`, `reason`, `statement`) ";
    query << "VALUES (NULL, " << BAN_IP << ", " << ip << ", " << mask << ", " << banTime << ", " << time(NULL) << ", "
        << gamemaster;
    query << ", " << db->escapeString(comment.c_str()) << ", " << reasonId << ", " << db->escapeString(
        statement.c_str()) << ")";
    return db->executeQuery(query.str());
}

bool IOBan::addPlayerBanishment(uint32_t playerId, int64_t banTime, uint32_t reasonId, ViolationAction_t actionId,
                                std::string comment, uint32_t gamemaster, PlayerBan_t type,
                                std::string statement/* = ""*/) const
{
    ...
    query <<
        "INSERT INTO `bans` (`id`, `type`, `value`, `param`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`, `statement`) ";
    query << "VALUES (NULL, " << BAN_PLAYER << ", " << playerId << ", " << type << ", " << banTime << ", " << time(NULL)
        << ", " << gamemaster;
    query << ", " << db->escapeString(comment.c_str()) << ", " << reasonId << ", " << actionId << ", " << db->
        escapeString(statement.c_str()) << ")";
    return db->executeQuery(query.str());
}
...
bool IOBan::addAccountBanishment(uint32_t account, int64_t banTime, uint32_t reasonId, ViolationAction_t actionId,
                                 std::string comment, uint32_t gamemaster, uint32_t playerId,
                                 std::string statement/* = ""*/) const
{
    ...
    query <<
        "INSERT INTO `bans` (`id`, `type`, `value`, `param`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`, `statement`) ";
    query << "VALUES (NULL, " << BAN_ACCOUNT << ", " << account << ", " << playerId << ", " << banTime << ", " <<
        time(NULL) << ", " << gamemaster;
    query << ", " << db->escapeString(comment.c_str()) << ", " << reasonId << ", " << actionId << ", " << db->
        escapeString(statement.c_str()) << ")";
    return db->executeQuery(query.str());
}
                        ...
                        std::string statement/* = ""*/) const
{
    ...
    query <<
        "INSERT INTO `bans` (`id`, `type`, `value`, `param`, `expires`, `added`, `admin_id`, `comment`, `reason`, `statement`) ";
    query << "VALUES (NULL, " << BAN_NOTATION << ", " << account << ", " << playerId << ", '-1', " << time(NULL) << ", "
        << gamemaster;
    query << ", " << db->escapeString(comment.c_str()) << ", " << reasonId << ", " << db->escapeString(
        statement.c_str()) << ")";
    return db->executeQuery(query.str());
...
bool IOBan::addStatement(uint32_t playerId, uint32_t reasonId,
                         std::string comment, uint32_t gamemaster, int16_t channelId/* = -1*/,
                         std::string statement/* = ""*/) const
{
    const auto db = Database::getInstance();
    DBQuery query;
    ...
    query << "`expires`, `added`, `admin_id`, `comment`, `reason`, `statement`) VALUES (NULL, " << BAN_STATEMENT << ", "
        << playerId;
    if(channelId >= 0)
        query << ", " << channelId;
...
bool IOBan::clearTemporials() const
{
    ...
    query << "UPDATE `bans` SET `active` = 0 WHERE `expires` <= " << time(NULL) <<
        " AND `expires` >= 0 AND `active` = 1" << db->getUpdateLimiter();
    return db->executeQuery(query.str());
}