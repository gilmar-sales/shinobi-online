...
bool GlobalEvents::registerEvent(Event *event, xmlNodePtr p, bool override)
{
    ...
    std::cout << "[Warning - GlobalEvents::configureEvent] Duplicate registered globalevent with name: " << globalEvent
        ->getName() << std::endl;
    return false;
}
...
bool GlobalEvent::configureEvent(xmlNodePtr p)
{
    ...
    else if(readXMLString(p, "time", strValue) || readXMLString(p, "at", strValue))
    {
        ...
        {
            std::cout << "[Error - GlobalEvent::configureEvent] No valid time \"" << strValue <<
                "\" for globalevent with name " << m_name << std::endl;
            return false;
        }
