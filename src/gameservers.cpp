...
bool GameServers::loadFromXml(bool showResult/* = true*/)
{
    ...
    while (p)
    {
        ...
        {
            name = "Server #" + id;
            std::cout << "[Warning - GameServers::loadFromXml] Missing name for server " << id << ", using default" <<
                std::endl;
        }
        ...
        else
        {
            ...
            std::cout << "[Warning - GameServers::loadFromXml] Missing versionMin for server " << id <<
        ...
        else
        {
            ...
            std::cout << "[Warning - GameServers::loadFromXml] Missing versionMax for server " << id <<
                ", using default" << std::endl;
        }
        ...
        {
            address = "localhost";
            std::cout << "[Warning - GameServers::loadFromXml] Missing address for server " << id << ", using default"
                << std::endl;
        }
        ...
        {
            port = 7171;
            std::cout << "[Warning - GameServers::loadFromXml] Missing port for server " << id << ", using default" <<
                std::endl;
        }
    ...
    if (showResult)
    {
        ...
        for (GameServersMap::iterator it = serverList.begin(); it != serverList.end(); it++)
            std::cout << it->second->getName() << " (" << it->second->getAddress() << ":" << it->second->getPort() <<
                ")" << std::endl;
    }
