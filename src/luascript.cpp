...
bool ScriptEnviroment::loadGameState()
{
    ...
    DBQuery query;
    query << "SELECT `key`, `value` FROM `global_storage` WHERE `world_id` = " << g_config.getNumber(
...
void ScriptEnviroment::getInfo(int32_t& scriptId, std::string& desc, LuaScriptInterface*& interface,
                               int32_t& callbackId, bool& timerEvent)
{
    scriptId = m_scriptId;
    desc = m_eventdesc;
...
void ScriptEnviroment::streamThing(std::stringstream& stream, const std::string& local, Thing* thing,
                                   uint32_t id/* = 0*/)
{
    if (!local.empty())
        stream << "local " << local << " = {" << std::endl;
...
void ScriptEnviroment::streamPosition(std::stringstream& stream, const std::string& local, const Position& position,
                                      uint32_t stackpos)
{
    if (!local.empty())
        stream << "local " << local << " = {" << std::endl;
...
bool LuaScriptInterface::initState()
{
    ...
    if (!loadDirectory(getFilePath(FILE_TYPE_OTHER, "lib/"), nullptr))
        std::cout << "[Warning - LuaScriptInterface::initState] Cannot load " << getFilePath(FILE_TYPE_OTHER, "lib/") <<
            std::endl;
...
bool LuaScriptInterface::closeState()
{
    ...
    {
        for (std::list<int32_t>::iterator lt = m_timerEvent.second.parameters.begin(); lt != m_timerEvent.second.
             parameters.end(); ++lt)
...
void LuaScriptInterface::executeTimer(uint32_t eventIndex)
{
    ...
    if (it != m_timerEvents.end())
    {
        ...
        //push parameters
        for (std::list<int32_t>::reverse_iterator rt = it->second.parameters.rbegin(); rt != it->second.parameters.
             rend(); ++rt)
...
void LuaScriptInterface::dumpStack(lua_State* L/* = nullptr*/)
{
    ...
    for (int32_t i = 1; i <= stack; ++i)
        std::cout << lua_typename(m_luaState, lua_type(m_luaState, -i)) << " " << lua_topointer(m_luaState, -i) <<
            std::endl;
}
...
std::string LuaScriptInterface::getGlobalString(lua_State* L, const std::string& _identifier,
...
int32_t LuaScriptInterface::getGlobalNumber(lua_State* L, const std::string& _identifier,
...
int32_t LuaScriptInterface::luaDoSetCreatureLight(lua_State* L)
{
    ...
    if (Creature* creature = env->getCreatureByUID(popNumber(L)))
    {
        Condition* condition = Condition::createCondition(CONDITIONID_COMBAT, CONDITION_LIGHT, time,
                                                          level | (color << 8));
...
int32_t LuaScriptInterface::luaIsInArray(lua_State* L)
{
    ...
    if (!lua_istable(L, -1))
    {
        ...
        if (type != data.type()) // check is it even same data type before searching deeper
            ...
            lua_pushboolean(
                L, boost::any_cast<std::string>(value) == asLowerCaseString(boost::any_cast<std::string>(data)));
...
int32_t LuaScriptInterface::luaAddEvent(lua_State* L)
{
    ...
    Scheduler::getInstance().addEvent(createSchedulerTask(delay, boost::bind(
                                                              &LuaScriptInterface::executeTimer, interface,
                                                              interface->m_lastEventTimerId)));
...
int32_t LuaScriptInterface::luaDoReloadInfo(lua_State* L)
{
    ...
    if (id >= RELOAD_FIRST && id <= RELOAD_LAST)
    {
        Scheduler::getInstance().addEvent(createSchedulerTask(SCHEDULER_MINTICKS,
                                                              boost::bind(
                                                                  &Game::reloadInfo, &g_game, (ReloadInfo_t)id, cid)));
        lua_pushboolean(L, true);
    }
    else
        lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoAddPlayerBanishment(lua_State* L)
{
    ...
    if (lua_isnumber(L, -1))
        lua_pushboolean(L, IOBan::getInstance()->addPlayerBanishment((uint32_t)popNumber(L),
                                                                     length, reason, action, comment, admin, type,
                                                                     statement));
    else
...
int32_t LuaScriptInterface::luaDoAddAccountBanishment(lua_State* L)
{
    ...
    lua_pushboolean(L, IOBan::getInstance()->addAccountBanishment((uint32_t)popNumber(L),
                                                                  length, reason, action, comment, admin, playerId,
                                                                  statement));
    return 1;
}
