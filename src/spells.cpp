...
bool Spells::registerEvent(Event* event, xmlNodePtr p, bool override)
{
    ...
    if (RuneSpell* rune = dynamic_cast<RuneSpell*>(event))
    {
        ...
        std::cout << "[Warning - Spells::registerEvent] Duplicate registered rune with id: " << rune->getRuneItemId() <<
            std::endl;
        return false;
    }
...
InstantSpell* Spells::getInstantSpell(const std::string words)
{
    ...
    if (result && words.length() > result->getWords().length())
    {
        ...
        if (param[0] != ' ' || (param.length() > 1 && (!result->getHasParam() || param.find(' ', 1) !=
            std::string::npos) && param[1] != '"'))
            return NULL;
    }
...
bool Spell::configureSpell(xmlNodePtr p)
{
    ...
    if (readXMLString(p, "name", strValue))
    {
        ...
        {
            "melee", "physical", "poison", "earth", "fire", "ice", "freeze", "energy", "drown", "death", "curse",
            "holy",
        ...
        for (uint32_t i = 0; i < sizeof(reservedList) / sizeof(const char*); ++i)
        {
            ...
            {
                std::cout << "Error: [Spell::configureSpell] Spell is using a reserved name: " << reservedList[i] <<
                    std::endl;
                return false;
            }
        }
    ...
    if (readXMLString(p, "blocktype", strValue))
    {
        ...
        if (tmpStrValue == "all")
            ...
            std::cout << "[Warning - Spell::configureSpell] Blocktype \"" << strValue << "\" does not exist." <<
                std::endl;
    }
...
bool InstantSpell::playerCastInstant(Player* player, const std::string& param)
{
    ...
    else if (needTarget || casterTargetOrDirection)
    {
        ...
        if (hasParam)
        {
            ...
            target = targetPlayer;
            if (limitRange && target && !Position::areInRange(Position(limitRange, limitRange, 0),
...
bool InstantSpell::SearchPlayer(const InstantSpell* spell, Creature* creature, const std::string& param)
{
    ...
    if (targetPlayer->hasCustomFlag(PlayerCustomFlag_NotSearchable) && !player->hasCustomFlag(
    ...
    std::stringstream ss;
    ss << targetPlayer->getName() << " " << g_game.getSearchString(player->getPosition(), targetPlayer->getPosition(),
                                                                   true, true) << ".";
        ...
        if (currentPos.z != blockedFloor)
        {
            ...
            if (!tmpTile || (tmpTile->ground == NULL && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID)))
            {
                ...
                if (tile && tmpTile && tmpTile->ground
                    ...
                    ret = g_game.internalMoveCreature(NULL, player, tile, tmpTile,
...
bool ConjureSpell::loadFunction(const std::string& functionName)
{
    ...
    {
        std::cout << "[Warning - ConjureSpell::loadFunction] Function \"" << functionName << "\" does not exist." <<
            std::endl;
        return false;
    }
...
ReturnValue ConjureSpell::internalConjureItem(Player* player, uint32_t conjureId, uint32_t conjureCount,
                                              ...
                                              slots_t slot/* = SLOT_WHEREVER*/, bool test/* = false*/)
{
    if (!transform)
    {
...
bool ConjureSpell::ConjureFood(const ConjureSpell* spell, Creature* creature, const std::string& param)
{
    ...
    if (internalConjureItem(player, foodType[random_range(0, (sizeof(foodType) / sizeof(uint32_t)) - 1)], 1) ==
        RET_NOERROR)
    {
...
bool RuneSpell::Illusion(const RuneSpell* spell, Creature* creature, Item* item, const Position& posFrom,
...
bool RuneSpell::Convince(const RuneSpell* spell, Creature* creature, Item* item, const Position& posFrom,
                         const Position& posTo)
{
    Player* player = creature->getPlayer();
    if (!player)
