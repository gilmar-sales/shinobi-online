...
bool IOGuild::getGuildId(uint32_t& id, const std::string& name)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guilds` WHERE `name` " << db->getStringComparison() << db->escapeString(name) <<
        " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " LIMIT 1";
...
bool IOGuild::getGuildById(std::string& name, uint32_t id)
{
    ...
    DBQuery query;
    query << "SELECT `name` FROM `guilds` WHERE `id` = " << id << " AND `world_id` = " << g_config.getNumber(
        ConfigManager::WORLD_ID) << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
...
bool IOGuild::swapGuildIdToOwner(uint32_t& value)
{
    ...
    DBQuery query;
    query << "SELECT `ownerid` FROM `guilds` WHERE `id` = " << value << " AND `world_id` = " << g_config.
        getNumber(ConfigManager::WORLD_ID) << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
...
bool IOGuild::guildExists(uint32_t guild)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guilds` WHERE `id` = " << guild << " AND `world_id` = " << g_config.getNumber(
        ConfigManager::WORLD_ID) << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
...
uint32_t IOGuild::getRankIdByName(uint32_t guild, const std::string& name)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guild_ranks` WHERE `guild_id` = " << guild << " AND `name` " << db->
        getStringComparison() << db->escapeString(name) << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return 0;
...
std::string IOGuild::getRank(uint32_t guid)
{
    ...
    DBQuery query;
    query << "SELECT `guild_ranks`.`name` FROM `players`, `guild_ranks` WHERE `players`.`id` = " << guid <<
        " AND `guild_ranks`.`id` = `players`.`rank_id` LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return "";
...
bool IOGuild::changeRank(uint32_t guild, const std::string& oldName, const std::string& newName)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guild_ranks` WHERE `guild_id` = " << guild << " AND `name` " << db->
        getStringComparison() << db->escapeString(oldName) << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
    ...
    query.str("");
    query << "UPDATE `guild_ranks` SET `name` = " << db->escapeString(newName) << " WHERE `id` = " << id << db->
        getUpdateLimiter();
...
bool IOGuild::createGuild(Player* player)
{
    ...
    DBQuery query;
    query << "INSERT INTO `guilds` (`id`, `world_id`, `name`, `ownerid`, `creationdata`, `motd`) VALUES (NULL, " <<
        g_config.getNumber(ConfigManager::WORLD_ID) << ", " << db->escapeString(player->getGuildName()) << ", " <<
        player->getGUID() << ", " << time(NULL) <<
...
bool IOGuild::joinGuild(Player* player, uint32_t guildId, bool creation/* = false*/)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guild_ranks` WHERE `guild_id` = " << guildId << " AND `level` = " << (
        creation ? "3" : "1") << " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
    ...
    query.str("");
    query << "UPDATE `players` SET `rank_id` = " << rankId << " WHERE `id` = " << player->getGUID() << db->
        getUpdateLimiter();
    if (!db->executeQuery(query.str()))
        return false;
...
bool IOGuild::disbandGuild(uint32_t guildId)
{
    ...
    DBQuery query;
    query << "UPDATE `players` SET `rank_id` = '' AND `guildnick` = '' WHERE `rank_id` = " <<
        getRankIdByLevel(guildId, GUILDLEVEL_LEADER) << " OR rank_id = " << getRankIdByLevel(guildId, GUILDLEVEL_VICE)
        << " OR rank_id = " << getRankIdByLevel(guildId, GUILDLEVEL_MEMBER);
    if (!db->executeQuery(query.str()))
        return false;
...
bool IOGuild::isInvited(uint32_t guild, uint32_t guid)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `guild_invites` WHERE `player_id` = " << guid << " AND `guild_id`= " << guild <<
        " LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return false;
...
uint32_t IOGuild::getGuildId(uint32_t guid)
{
    ...
    DBQuery query;
    query << "SELECT `guild_ranks`.`guild_id` FROM `players`, `guild_ranks` WHERE `players`.`id` = " << guid <<
        " AND `guild_ranks`.`id` = `players`.`rank_id` LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
        return 0;
...
GuildLevel_t IOGuild::getGuildLevel(uint32_t guid)
{
    ...
    DBQuery query;
    query << "SELECT `guild_ranks`.`level` FROM `players`, `guild_ranks` WHERE `players`.`id` = " << guid <<
        " AND `guild_ranks`.`id` = `players`.`rank_id` LIMIT 1";
    if (!(result = db->storeQuery(query.str())))
...
bool IOGuild::setGuildLevel(uint32_t guid, GuildLevel_t level)
{
    ...
    query.str("");
    query << "UPDATE `players` SET `rank_id` = " << result->getDataInt("id") << " WHERE `id` = " << guid << db->
        getUpdateLimiter();
    result->free();
    return db->executeQuery(query.str());
}
...
bool IOGuild::setGuildNick(uint32_t guid, const std::string& nick)
{
    const auto db = Database::getInstance();
    DBQuery query;
    query << "UPDATE `players` SET `guildnick` = " << db->escapeString(nick) << " WHERE `id` = " << guid << db->
        getUpdateLimiter();
    return db->executeQuery(query.str());
}
...
bool IOGuild::setMotd(uint32_t guild, const std::string& newMessage)
{
    const auto db = Database::getInstance();
    DBQuery query;
    query << "UPDATE `guilds` SET `motd` = " << db->escapeString(newMessage) << " WHERE `id` = " << guild << db->
        getUpdateLimiter();
    return db->executeQuery(query.str());
}
