...
bool BaseEvents::loadFromXml()
{
    ...
    const xmlDocPtr doc = xmlParseFile(
        getFilePath(FILE_TYPE_OTHER, std::string(scriptsName + "/" + scriptsName + ".xml")).c_str());
...
bool BaseEvents::parseEventNode(xmlNodePtr p, const std::string& scriptsPath, bool override)
{
    Event* event = getEvent(reinterpret_cast<const char*>(p->name));
...
bool Event::loadScript(const std::string& script, bool file)
{
    ...
    if (!file)
    {
        ...
        if (buffer.find(function) == std::string::npos)
        {
            ...
            scriptstream << function << "(" << getScriptEventParams() << ")" << std::endl << buffer << std::endl <<
                "end";
    ...
    if (id == -1)
    {
        std::cout << "[Warning - Event::loadScript] Event " << getScriptEventName() << " not found (" << script << ")"
            << std::endl;
        return false;
    }
