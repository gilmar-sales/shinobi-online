...
inline void Actions::clearMap(ActionMap &map) {
    map.clear();
}
...
bool Actions::registerEvent(Event_Ptr event, const xmlNodePtr p, const bool override) {
    const auto action = boost::dynamic_pointer_cast<Action>(event);
    ...
    if (readXMLString(p, "itemid", strValue)) {
        ...
        for (size_t i = 1, size = intVector.size(); i < size; ++i) {
            ...
            useItemMap[intVector[i]] = boost::make_shared<Action>(action);
        }
    }
    ...
    if (readXMLString(p, "uniqueid", strValue)) {
        ...
        for (size_t i = 1, size = intVector.size(); i < size; ++i) {
            if (uniqueItemMap.find(intVector[i]) != uniqueItemMap.end()) {
                if (!override) {
                    std::cout << "[Warning - Actions::registerEvent] Duplicate registered item uid: " << intVector[i] <<
                            std::endl;
                    continue;
                } else
            ...
            uniqueItemMap[intVector[i]] = boost::make_shared<Action>(action);;
        }
    }
        ...
        if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size()) {
            for (size_t i = 0, size = intVector.size(); i < size; ++i) {
                ...
                while (intVector[i] <= endIntVector[i]) {
                    if (uniqueItemMap.find(intVector[i]) != uniqueItemMap.end()) {
                        if (!override) {
                            std::cout << "[Warning - Actions::registerEvent] Duplicate registered item with uid: " <<
                                    intVector[i] <<
    ...
    if (readXMLString(p, "actionid", strValue)) {
        ...
        for (size_t i = 1, size = intVector.size(); i < size; ++i) {
            if (actionItemMap.find(intVector[i]) != actionItemMap.end()) {
                if (!override) {
                    std::cout << "[Warning - Actions::registerEvent] Duplicate registered item aid: " << intVector[i] <<
                            std::endl;
                    continue;
                } else
            ...
            actionItemMap[intVector[i]] = boost::make_shared<Action>(action);;
        }
    }
        ...
        if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size()) {
            for (size_t i = 0, size = intVector.size(); i < size; ++i) {
                ...
                while (intVector[i] <= endIntVector[i]) {
                    if (actionItemMap.find(intVector[i]) != actionItemMap.end()) {
                        if (!override) {
                            std::cout << "[Warning - Actions::registerEvent] Duplicate registered item with aid: " <<
                                    intVector[i] <<
...
ReturnValue Actions::canUseFar(const Creature *creature, const Position &toPos, bool checkLineOfSight) {
 ...
 Action* Actions::getAction(const Item *item, ActionType_t type/* = ACTION_ANY*/) const {
    ...
    if (type == ACTION_ANY || type == ACTION_RUNEID) {
        if (const auto runeSpell = g_spells->getRuneSpell(item->getID()).get())
            return runeSpell;
    }
...
bool Actions::useItem(Player *player, const Position &pos, uint8_t index, Item *item) {
    ...
    player->setNextAction(
        OTSYS_TIME() + g_config.getNumber(ConfigManager::ACTIONS_DELAY_INTERVAL) - SCHEDULER_MINTICKS);
...
bool Actions::useItemEx(Player *player, const Position &fromPos, const Position &toPos,
    ...
    player->stopWalk();
    player->setNextAction(
        OTSYS_TIME() + g_config.getNumber(ConfigManager::EX_ACTIONS_DELAY_INTERVAL) - SCHEDULER_MINTICKS);
...
Action::Action(const Action_Ptr &copy) : Event(copy) {
...
bool Action::loadFunction(const std::string &functionName) {
    if (const std::string tmpFunctionName = asLowerCaseString(functionName); tmpFunctionName == "increaseitemid")
...
bool Action::increaseItemId(Player *player, Item *item, const PositionEx &posFrom, const PositionEx &posTo,
...
bool Action::decreaseItemId(Player *player, Item *item, const PositionEx &posFrom, const PositionEx &posTo,
                            bool extendedUse, uint32_t creatureId) {
    if (!player || !item)
        return false;
...
bool Action::executeUse(Player *player, Item *item, const PositionEx &fromPos, const PositionEx &toPos,
                        bool extendedUse, uint32_t creatureId) {
    ...
    if (m_interface->reserveEnv()) {
        ...
        if (m_scripted == EVENT_SCRIPT_BUFFER) {
            ...
            if (m_interface->loadBuffer(scriptstream.str())) {
                ...
                result = LuaScriptInterface::getGlobalBool(L, "_result", true);
            }

            LuaScriptInterface::releaseEnv();
            return result;
        } else {
