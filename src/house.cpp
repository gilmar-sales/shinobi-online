...
House::House(uint32_t houseId)
{
    ...
    syncFlags = HOUSE_SYNC_NAME | HOUSE_SYNC_TOWN | HOUSE_SYNC_SIZE | HOUSE_SYNC_PRICE | HOUSE_SYNC_RENT |
        HOUSE_SYNC_GUILD;
}
...
void House::updateDoorDescription(std::string _name/* = ""*/)
{
    ...
    if (owner)
    {
        ...
        sprintf(houseDescription, "It belongs to %s '%s'. %s owns this %s.", tmp.c_str(), name.c_str(), _name.c_str(),
                tmp.c_str());
    }
    else
        sprintf(houseDescription, "It belongs to %s '%s'. Nobody owns this %s. It costs %d gold coins.", tmp.c_str(),
                name.c_str(), tmp.c_str(), price);
...
bool House::transferToDepot()
{
    ...
    if (player)
    {
        ...
        for (ItemList::iterator it = moveList.begin(); it != moveList.end(); ++it)
            g_game.internalMoveItem(NULL, (*it)->getParent(), depot, INDEX_WHEREEVER, (*it), (*it)->getItemCount(),
                                    NULL, FLAG_NOLIMIT);
...
TransferItem* TransferItem::createTransferItem(House* house)
{
    ...
    char buffer[150];
    sprintf(buffer, "It is a %s transfer document for '%s'.", house->isGuild() ? "guild hall" : "house",
            house->getName().c_str());
...
bool AccessList::parseList(const std::string& _list)
{
    ...
    while (getline(listStream, line))
    {
        ...
        else if (line.find("!") != std::string::npos || line.find("*") != std::string::npos || line.find("?") !=
            std::string::npos)
            addExpression(line);
        else
            addPlayer(line);
...
bool AccessList::addGuild(const std::string& guildName, const std::string& rankName)
{
    ...
    if (!rankId && (tmp.find("?") == std::string::npos || tmp.find("!") == std::string::npos || tmp.find("*") ==
        std::string::npos))
        rankId = -1;
