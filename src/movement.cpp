...
bool MoveEvents::registerEvent(Event* event, xmlNodePtr p, bool override)
{
    ...
    if (readXMLString(p, "fromid", strValue) && readXMLString(p, "toid", endStrValue))
    {
        ...
        if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
        {
            ...
            std::cout << "[Warning - MoveEvents::registerEvent] Malformed entry (from item: \"" << strValue <<
    ...
    if (readXMLString(p, "fromuid", strValue) && readXMLString(p, "touid", endStrValue))
    {
        ...
        if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
        {
            ...
            std::cout << "[Warning - MoveEvents::registerEvent] Malformed entry (from unique: \"" << strValue <<
    ...
    if (readXMLString(p, "fromaid", strValue) && readXMLString(p, "toaid", endStrValue))
    {
        ...
        if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
        {
            ...
            std::cout << "[Warning - MoveEvents::registerEvent] Malformed entry (from action: \"" << strValue <<
...
uint32_t MoveEvents::onCreatureMove(Creature* actor, Creature* creature, const Tile* fromTile, const Tile* toTile,
                                    bool isStepping)
{
...
bool MoveEvent::configureEvent(xmlNodePtr p)
{
    ...
    if (readXMLString(p, "type", strValue) || readXMLString(p, "event", strValue))
    {
        ...
        {
            std::cout << "[Error - MoveEvent::configureMoveEvent] Unknown event type \"" << strValue << "\"" <<
                std::endl;
            return false;
        }
        ...
        if (m_eventType == MOVE_EVENT_EQUIP || m_eventType == MOVE_EVENT_DEEQUIP)
        {
            if (readXMLString(p, "slot", strValue))
            {
                ...
                if (tmpStrValue == "head")
                    ...
                    std::cout << "[Warning - MoveEvent::configureMoveEvent] Unknown slot type \"" << strValue << "\"" <<
                        std::endl;
            }
...
uint32_t MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
    ...
    {
        if (player->getLevel() < (uint32_t)moveEvent->getReqLevel() || player->getMagicLevel() < (uint32_t)moveEvent->
            getReqMagLv())
            return 0;
        ...
        if (!moveEvent->getVocEquipMap().empty() && moveEvent->getVocEquipMap().find(player->getVocationId()) ==
            moveEvent->getVocEquipMap().end())
            return 0;
    }
    ...
    for (uint32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i)
    {
        ...
        if (it.abilities.skillsPercent[i])
        {
            player->setVarSkill((skills_t)i,
                                (int32_t)(player->getSkill((skills_t)i, SKILL_LEVEL) * ((it.abilities.skillsPercent[i] -
                                    100) / 100.f)));
            if (!needUpdateSkills)
                needUpdateSkills = true;
        }
    ...
    for (uint32_t s = STAT_FIRST; s <= STAT_LAST; ++s)
    {
        ...
        if (it.abilities.statsPercent[s])
        {
            player->setVarStats(
                (stats_t)s,
                (int32_t)(player->getDefaultStats((stats_t)s) * ((it.abilities.statsPercent[s] - 100) / 100.f)));
            if (!needUpdateStats)
                needUpdateStats = true;
        }
    }
