...
bool Vocations::parseVocationNode(xmlNodePtr p)
{
    ...
    for (xmlNodePtr configNode = p->children; configNode; configNode = configNode->next)
    {
        if (!xmlStrcmp(configNode->name, (const xmlChar*)"skill"))
        {
            ...
            if (readXMLInteger(configNode, "distanceBase", intValue) ||
                readXMLInteger(configNode, "distBase", intValue))
            ...
            if (readXMLInteger(configNode, "shieldingBase", intValue) || readXMLInteger(
                configNode, "shieldBase", intValue))
            ...
            if (readXMLInteger(configNode, "id", intValue))
            {
                ...
                {
                    std::cout << "[Error - Vocations::parseVocationNode] No valid skill id (" << intValue << ")." <<
                        std::endl;
                    continue;
                }
        ...
        else if (!xmlStrcmp(configNode->name, (const xmlChar*)"formula"))
        {
            ...
            if (readXMLFloat(configNode, "distDamage", floatValue) || readXMLFloat(
                configNode, "distanceDamage", floatValue))
            ...
            if (readXMLFloat(configNode, "magDamage", floatValue) ||
                readXMLFloat(configNode, "magicDamage", floatValue))
                voc->setMultiplier(MULTIPLIER_MAGIC, floatValue);

            if (readXMLFloat(configNode, "magHealingDamage", floatValue) || readXMLFloat(
                configNode, "magicHealingDamage", floatValue))
            ...
            if (readXMLFloat(configNode, "magDefense", floatValue) || readXMLFloat(
                configNode, "magicDefense", floatValue))
