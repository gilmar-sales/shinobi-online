    ...
    if (mod.enabled)
    {
        ...
        while (p)
        {
            if (!xmlStrcmp(p->name, (const xmlChar*)"quest"))
                ...
                Vocations::getInstance()->parseVocationNode(p);
                //duplicates checking is dangerous, shouldn't be performed
            ...
            else if (!xmlStrcmp(p->name, (const xmlChar*)"lib") || !xmlStrcmp(p->name, (const xmlChar*)"config"))
            {
                ...
                {
                    std::cout << "[Warning - ScriptingManager::loadFromXml] Lib without name in mod " << strValue <<
                        std::endl;
                    p = p->next;
                    continue;
                }
