...
bool Outfits::parseOutfitNode(xmlNodePtr p)
{
    ...
    for(xmlNodePtr listNode = p->children; listNode != NULL; listNode = listNode->next)
    {
        ...
        {
            std::cout << "[Error - Outfits::parseOutfitNode] Missing looktype for an outfit with id " << outfit.outfitId
                << std::endl;
            continue;
        }

        outfit.lookType = intValue;
        if(!readXMLString(listNode, "gender", strValue) && !readXMLString(listNode, "type", strValue) && !readXMLString(
               listNode, "sex", strValue))
        {
            std::cout << "[Error - Outfits::parseOutfitNode] Missing gender(s) for an outfit with id " << outfit.
                outfitId
        ...
        {
            std::cout << "[Error - Outfits::parseOutfitNode] Invalid gender(s) for an outfit with id " << outfit.
                outfitId
                << " and looktype " << outfit.lookType << std::endl;
            continue;
        }
        ...
        if(readXMLString(listNode, "requirement", strValue))
        {
            ...
            if(tmpStrValue == "none")
                ...
                std::cout << "[Warning - Outfits::loadFromXml] Unknown requirement tag value, using default (both)" <<
                    std::endl;
        }
        ...
        for(xmlNodePtr configNode = listNode->children; configNode != NULL; configNode = configNode->next)
        {
            if(!xmlStrcmp(configNode->name, (const xmlChar *)"reflect"))
            {
                ...
                if(readXMLInteger(configNode, "percentPoison", intValue) || readXMLInteger(
                       configNode, "percentEarth", intValue))
                    outfit.reflect[REFLECT_PERCENT][COMBAT_EARTHDAMAGE] += intValue;
                ...
                if(readXMLInteger(configNode, "chancePoison", intValue) || readXMLInteger(
                       configNode, "chanceEarth", intValue))
                    outfit.reflect[REFLECT_CHANCE][COMBAT_EARTHDAMAGE] += intValue;
            ...
            else if(!xmlStrcmp(configNode->name, (const xmlChar *)"skills"))
            {
                ...
                if(readXMLInteger(configNode, "distancePercent", intValue) || readXMLInteger(
                       configNode, "distPercent", intValue))
                    outfit.skillsPercent[SKILL_DIST] += intValue;

                if(readXMLInteger(configNode, "shieldingPercent", intValue) || readXMLInteger(
                       configNode, "shieldPercent", intValue))
                    outfit.skillsPercent[SKILL_SHIELD] = intValue;

                if(readXMLInteger(configNode, "fishingPercent", intValue) || readXMLInteger(
                       configNode, "fishPercent", intValue))
                ...
                if(readXMLInteger(configNode, "weaponPercent", intValue) || readXMLInteger(
                       configNode, "weaponsPercent", intValue))
                {
                    outfit.skillsPercent[SKILL_CLUB] += intValue;
                    outfit.skillsPercent[SKILL_SWORD] += intValue;
                    outfit.skillsPercent[SKILL_AXE] += intValue;
        ...
        for(IntegerVec::iterator it = intVector.begin(); it != intVector.end(); ++it)
        {
            ...
            if(fit != outfitsMap[(*it)].end())
            {
                if(override)
                {
                    ...
                    std::cout << "[Warning - Outfits::parseOutfitNode] Duplicated outfit for gender " << (*it) <<
                        " with lookType " << outfit.outfitId << std::endl;
            }
            else
            {
...
bool Outfits::addAttributes(uint32_t playerId, uint32_t outfitId, uint16_t sex, uint16_t addons)
{
    ...
    for(uint32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i)
    {
        ...
        {
            needUpdateSkills = true;
            player->setVarSkill((skills_t)i,
                                (int32_t)(player->getSkill((skills_t)i, SKILL_LEVEL) * (
                                              (outfit.skillsPercent[i] - 100) / 100.f)));
        }
    }
    ...
    for(uint32_t s = STAT_FIRST; s <= STAT_LAST; ++s)
    {
        ...
        if(outfit.statsPercent[s])
        {
            needUpdateStats = true;
            player->setVarStats(
                (stats_t)s, (int32_t)(player->getDefaultStats((stats_t)s) * ((outfit.statsPercent[s] - 100) / 100.f)));
        }
    }
