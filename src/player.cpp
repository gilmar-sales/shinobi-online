...
Item* Player::getWeapon(bool ignoreAmmo /*= false*/)
{
	...
	for(uint32_t slot = SLOT_RIGHT; slot <= SLOT_LEFT; slot++)
	{
		...
		switch(item->getWeaponType())
		{
			...
			case WEAPON_FIST:
			{
				const Weapon_Ptr weapon = g_weapons->getWeapon(item);
				if(weapon)
					return item;
				break;
			}
			...
			case WEAPON_DIST:
			{
				if(!ignoreAmmo && item->getAmmoType() != AMMO_NONE)
				{
					...
					{
						const Weapon_Ptr weapon = g_weapons->getWeapon(ammoItem);
						if(weapon)
						{
...
void Player::setSkill(skills_t skill, uint32_t level)
{
	if(!level)
		return;
