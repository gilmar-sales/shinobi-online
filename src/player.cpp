...
void Player::onAddCondition(ConditionType_t type, bool hadCondition)
{
    ...
    if (getLastPosition().x && type !=
        ...
        // don't send if player have just logged in (its already done in protocolgame), or condition have no icons
        sendIcons();
}
...
bool Player::addUnjustifiedKill(const Player* attacked)
{
    ...
    if (!g_config.getBool(ConfigManager::USE_BLACK_SKULL))
    {
        ...
        if (!IOBan::getInstance()->addAccountBanishment(accountId, (now + g_config.getNumber(
                                                            ConfigManager::KILLS_BAN_LENGTH)), 20, ACTION_BANISHMENT,
                                                        "Unjustified player killing.", 0, guid))
            return true;
