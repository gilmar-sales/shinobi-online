...
bool IOMapSerialize::loadMap(boost::shared_ptr<Map> map)
{
...
bool IOMapSerialize::saveMap(boost::shared_ptr<Map> map)
{
    bool saved = false;
    ...
    return saved && saveMapRelational(map);
}
...
bool IOMapSerialize::loadHouses()
{
    ...
    for(HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it != Houses::getInstance()->getHouseEnd(); ++
        it)
    {
...
bool IOMapSerialize::updateHouses()
{
        ...
        it)
    {
        ...
        else
        {
            query.str("");
            query <<
                "INSERT INTO `houses` (`id`, `world_id`, `owner`, `name`, `town`, `size`, `price`, `rent`, `doors`, `beds`, `tiles`, `guild`) VALUES ("
...
bool IOMapSerialize::loadMapRelational(boost::shared_ptr<Map> map)
{
    const auto db = Database::getInstance();
        ...
        it)
    {
        ...
        if(DBResult *result = db->storeQuery(query.str()))
        {
            ...
            {
                query.str("");
                query << "SELECT * FROM `tile_items` WHERE `tile_id` = " << result->getDataInt("id") <<
                    " AND `world_id` = "
...
bool IOMapSerialize::saveMapRelational(boost::shared_ptr<Map> map)
{
    const auto db = Database::getInstance();
...
bool IOMapSerialize::loadMapBinary(boost::shared_ptr<Map> map)
{
    const auto db = Database::getInstance();
    DBResult *result;

    DBQuery query;
    query << "SELECT `house_id`, `data` FROM `house_data` WHERE `world_id` = " << g_config.getNumber(
        ConfigManager::WORLD_ID);
    if(!(result = db->storeQuery(query.str())))
        return false;
...
bool IOMapSerialize::saveMapBinary(boost::shared_ptr<Map> map)
{
    const auto db = Database::getInstance();
    //Start the transaction
...
bool IOMapSerialize::loadItems(const boost::shared_ptr<Database> db, DBResult *result, Cylinder *parent,
                               bool depotTransfer/* = false*/)
{
    ItemMap itemMap;
    Tile *tile = NULL;
    if(!parent->getItem())
    ...
    do
    {
        ...
        if(iType.moveable || iType.forceSerialize || pid)
        {
            ...
            if(item->unserializeAttr(propStream))
            {
                ...
                std::cout << "[Warning - IOMapSerialize::loadItems] Unserialization error [0] for item type " << id <<
                    std::endl;
        }
        else if(tile)
        {
        ...
        if(item)
        {
            if(item->unserializeAttr(propStream))
            {
                ...
                std::cout << "[Warning - IOMapSerialize::loadItems] Unserialization error [1] for item type " << id <<
                    std::endl;
        }
...
bool IOMapSerialize::saveItems(const boost::shared_ptr<Database> db, uint32_t &tileId, uint32_t houseId,
                               const Tile *tile)
{
    ...
    for(int32_t i = 0; i < thingCount; ++i)
    {
        ...
        query << tileId << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << ++runningId << ", " <<
            parentId << ", "
            << item->getID() << ", " << (int32_t)item->getSubType() << ", " << db->escapeBlob(
...
bool IOMapSerialize::loadContainer(PropStream &propStream, Container *container)
{
    while(container->serializationCount > 0)
    {
        ...
        {
            std::cout << "[Warning - IOMapSerialize::loadContainer] Unserialization error [0] for item in container " <<
    ...
    std::cout << "[Warning - IOMapSerialize::loadContainer] Unserialization error [1] for item in container " <<
        container->getID() << std::endl;
    return false;
}
...
bool IOMapSerialize::loadItem(PropStream &propStream, Cylinder *parent, bool depotTransfer/* = false*/)
{
    ...
    if(iType.moveable || iType.forceSerialize || (!depotTransfer && !tile))
    {
        ...
        if(!item->unserializeAttr(propStream))
        {
            std::cout << "[Warning - IOMapSerialize::loadItem] Unserialization error [0] for item type " << id <<
                std::endl;
            delete item;
            return false;
        }
    ...
    if(item)
    {
        if(item->unserializeAttr(propStream))
        {
            ...
            std::cout << "[Warning - IOMapSerialize::loadItem] Unserialization error [1] for item type " << id <<
                std::endl;
...
bool IOMapSerialize::saveItem(PropWriteStream &stream, const Item *item)
{
    ...
    if(const Container *container = item->getContainer())
    {
        ...
        for(ItemList::const_reverse_iterator rit = container->getReversedItems(); rit != container->getReversedEnd();
            ++rit)
            saveItem(stream, (*rit));
    }
