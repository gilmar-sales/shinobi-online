...
Game::Game(): services(nullptr), lastMotdId(0), lastHighscoreCheck(0)
{
...
void Game::start(ServiceManager *servicer)
{
    ...
    {
        int32_t prepareGlobalSaveHour = g_config.getNumber(ConfigManager::GLOBALSAVE_H) - 1, hoursLeft = 0, minutesLeft
                    = 0, minutesToRemove = 0;
...
void Game::setGameState(GameState_t newState)
{
    ...
    if(gameState != newState)
    {
        ...
        case GAME_STATE_INIT: {
            ...
            auto _ = IOBan::getInstance()->clearTemporials();
...
void Game::saveGameState(bool shallow)
{
    ...
    auto map_serialize = IOMapSerialize::getInstance();
    map_serialize->saveMap(map);
    ...
    std::cout << "> SAVE: Complete in " << (OTSYS_TIME() - start) / (1000.) << " seconds using " << storage <<
...
Player *Game::getPlayerByID(uint32_t id)
{
    ...
    auto it = Player::autoList.find(id);
...
Creature *Game::getCreatureByName(std::string s)
{
    ...
    toLowerCaseString(s);
    for(auto it = autoList.begin(); it != autoList.end(); ++it)
    {
...
Player *Game::getPlayerByName(std::string s)
{
    ...
    toLowerCaseString(s);
    for(auto it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
    {
        if(!it->second->isRemoved() && asLowerCaseString(it->second->getName()) == s)
            return it->second;
    }
...
bool Game::placeCreature(Creature *creature, const Position &pos, bool extendedPos /*= false*/, bool forced /*= false*/)
{
    ...
    {
        for(ConditionList::iterator it = tmpPlayer->storedConditionList.begin(); it != tmpPlayer->storedConditionList.
                end(); ++it)
        {
    ...
    creature->getParent()->
              postAddNotification(NULL, creature, NULL, creature->getParent()->__getIndexOfThing(creature));
                              ...
                              const Position &movingCreaturePos, const Position &toPos)
{
    ...
    if(!player->canDoAction())
    {
        ...
        SchedulerTask *task = createSchedulerTask(delay, boost::bind(&Game::playerMoveCreature,
                                                                     this, playerId, movingCreatureId,
    ...
    if(!Position::areInRange<1, 1, 0>(movingCreaturePos, player->getPosition()) && !player->hasCustomFlag(
           PlayerCustomFlag_CanMoveFromFar))
    {
        ...
        if(getPathToEx(player, movingCreaturePos, listDir, 0, 1, true, true))
        {
            ...
            SchedulerTask *task = createSchedulerTask(player->getStepDuration(), boost::bind(
                                                          &Game::playerMoveCreature, this,
    ...
    if((!movingCreature->isPushable() && !player->hasFlag(PlayerFlag_CanPushAllCreatures)) || !player->
       canSeeCreature(movingCreature))
    {
    ...
    if(!player->hasCustomFlag(PlayerCustomFlag_CanThrowAnywhere) && ((std::abs(pos.x - toPos.x) > movingCreature->
                                                                      getThrowRange()) || (std::abs(
                                                                             pos.y - toPos.y) > movingCreature->
                                                                         getThrowRange()) || (std::abs(
                                                                             pos.z - toPos.z) * 4 > movingCreature->
                                                                         getThrowRange())))
    {
    ...
    if(ret != RET_NOERROR)
    {
        if(!player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar) || !player->hasCustomFlag(
        ...
        if(currentPos.z != 8 && creature->getTile()->hasHeight(3)) //try go up
        {
               ...
               && tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))
            {
                ...
                destPos.z--;
                                                                             ...
                                                                             !tmpTile->hasProperty(BLOCKSOLID))) && (
                    tmpTile = map->getTile(Position(
...
ReturnValue Game::internalMoveCreature(Creature *actor, Creature *creature, Cylinder *fromCylinder,
                                       Cylinder *toCylinder, uint32_t flags/* = 0*/)
{
                          ...
                          uint16_t spriteId, int16_t fromStackpos, const Position &toPos, uint8_t count)
{
    ...
    if(!player->canDoAction())
    {
        ...
        SchedulerTask *task = createSchedulerTask(delay, boost::bind(&Game::playerMoveItem, this,
                                                                     playerId, fromPos, spriteId, fromStackpos, toPos,
                                                                     count));
    ...
    if(!Position::areInRange<1, 1, 0>(playerPos, mapFromPos) && !player->
       hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
    {
    ...
    if(item->isHangable() && toCylinder->getTile()->hasProperty(SUPPORTHANGABLE))
    {
        ...
        if(!Position::areInRange<1, 1, 0>(playerPos, mapToPos) && !player->hasCustomFlag(
               PlayerCustomFlag_CanMoveFromFar))
        {
               ...
               && !Position::areInRange<1, 1, 0>(mapFromPos, walkPos))
            {
                ...
                Item *moveItem  = NULL;
                ReturnValue ret = internalMoveItem(player, fromCylinder, player, INDEX_WHEREEVER, item, count,
                                                   &moveItem);
                if(ret != RET_NOERROR)
                {
                    player->sendCancelMessage(ret);
                                   ...
                                   int32_t index, Item *item, uint32_t count, Item **_moveItem, uint32_t flags /*= 0*/)
{
    ...
    if(ret == RET_NEEDEXCHANGE)
    {
        ...
        if(ret == RET_NOERROR)
        {
            ...
            ReturnValue retExchangeMaxCount = fromCylinder->__queryMaxCount(
                -1, toItem, toItem->getItemCount(), maxExchangeQueryCount, 0);
...
ReturnValue Game::internalAddItem(Creature *actor, Cylinder *toCylinder, Item *item,
...
ReturnValue Game::internalTeleport(Thing *thing, const Position &newPos, bool pushMove, uint32_t flags /*= 0*/)
{
    ...
    if(Tile *toTile = map->getTile(newPos))
    {
        ...
        if(Item *item = thing->getItem())
            return internalMoveItem(NULL, item->getParent(), toTile, INDEX_WHEREEVER, item, item->getItemCount(), NULL,
                                    flags);
    }
                           ...
                           const Position &toPos, int16_t toStackpos, uint16_t toSpriteId, bool isHotkey)
{
    ...
    if(!player->canDoAction())
    {
        ...
        SchedulerTask *task = createSchedulerTask(delay, boost::bind(&Game::playerUseItemEx, this,
                                                                     playerId, fromPos, fromStackpos, fromSpriteId,
        ...
        if(ret == RET_TOOFARAWAY)
        {
            ...
            if(getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true))
            {
                ...
                SchedulerTask *task = createSchedulerTask(400, boost::bind(&Game::playerUseBattleWindow, this,
                                                                           playerId, fromPos, fromStackpos, creatureId,
                                                                           spriteId, isHotkey));
...
bool Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid)
{
    ...
    player->sendContainer(cid, parentContainer,
                          (dynamic_cast<const Container *>(parentContainer->getParent()) != NULL));
    return true;
}
                              ...
                              uint32_t tradePlayerId, uint16_t spriteId)
{
    ...
    if(!tradeItem || tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() || (tradeItem->
           isLoadedFromMap() &&
...
{
    if(player->tradeState != TRADE_NONE && !(player->tradeState == TRADE_ACKNOWLEDGE && player->tradePartner ==
                                             tradePartner))
    {
...
bool Game::playerAcceptTrade(uint32_t playerId)
{
    ...
    if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
    {
        ...
        if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
        {
            ...
            internalMoveItem(player, cylinder1, tradePartner, INDEX_WHEREEVER, tradeItem1, tradeItem1->getItemCount(),
                             NULL);
            internalMoveItem(tradePartner, cylinder2, player, INDEX_WHEREEVER, tradeItem2, tradeItem2->getItemCount(),
                             NULL);
...
bool Game::playerLookAt(uint32_t playerId, const Position &pos, uint16_t spriteId, int16_t stackpos)
{
    ...
    if(player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
    {
        if(const Creature *creature = thing->getCreature())
        {
            ...
            {
                ss << std::endl << "IP: " << convertIPAddress(destPlayer->getIP()) << ", Client: " << destPlayer->
                    getClientVersion() << ".";
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string &receiver,
                     const std::string &text)
{
    ...
    if(!muted)
    {
        ...
        if(!muteTime)
        {
            ...
            if(ret == RET_NOERROR || (ret == RET_NEEDEXCHANGE && !g_config.
                                      getBool(ConfigManager::BUFFER_SPELL_FAILURE)))
                return true;
        }
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature *attacker, Creature *target, int32_t healthChange,
                              ...
                              TextColor_t hitColor/* = TEXTCOLOR_UNKNOWN*/, bool force/* = false*/)
{
    const Position &targetPos = target->getPosition();
    if(healthChange > 0)
    {
        ...
        for(auto it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++it)
        {
        ...
        if(damage != 0)
        {
            if(target->hasCondition(CONDITION_MANASHIELD) && combatType != COMBAT_UNDEFINEDDAMAGE)
            {
                ...
                if(manaDamage != 0)
                {
                    bool deny                           = false;
                    CreatureEventList statsChangeEvents = target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);
                    for(CreatureEventList::iterator it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++
                        it)
                    {
...
bool Game::combatChangeMana(Creature *attacker, Creature *target, int32_t manaChange)
{
    ...
    else
    {
        ...
        if(manaLoss > 0)
        {
            ...
            const auto statsChangeEvents = target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);

            for(const auto stateChangeEvent: statsChangeEvents)
            {
                if(!stateChangeEvent->executeStatsChange(target, attacker, STATSCHANGE_MANALOSS,
                                                         COMBAT_UNDEFINEDDAMAGE, manaChange))
                    deny = true;
            }
...
{
    for(const auto spectator: list)
    {
        if(const auto player = spectator->getPlayer(); player)
...
void Game::internalDecayItem(Item *item)
{
    ...
    else
    {
        ReturnValue ret = internalRemoveItem(NULL, item);
        if(ret != RET_NOERROR)
            std::cout << "> DEBUG: internalDecayItem failed, error code: " << (int32_t)ret << ", item id: " << item->
                getID() << std::endl;
    }
}
                                 ...
                                 std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
    ...
    if(ipBanishment && target->getIP())
    {
        ...
        IOBan::getInstance()->addIpBanishment(target->getIP(), length[pos], reason, comment, player->getGUID(),
                                              0xFFFFFFFF);
    }
    ...
    {
    case ACTION_NOTATION: {
        ss << " (" << (g_config.getNumber(ConfigManager::NOTATIONS_TO_BAN) - IOBan::getInstance()->
                       getNotationsCount(
...
std::string Game::getSearchString(const Position fromPos, const Position toPos, bool fromIsCreature/* = false*/,
...
Highscore Game::getHighscore(uint16_t skill)
{
    ...
    {
        if(skill == SKILL__MAGLEVEL)
            query << "SELECT `maglevel`, `name` FROM `players` ORDER BY `maglevel` DESC, `manaspent` DESC LIMIT " <<
                g_config.getNumber(ConfigManager::HIGHSCORES_TOP);
        else
            query << "SELECT `level`, `name` FROM `players` ORDER BY `level` DESC, `experience` DESC LIMIT " << g_config
                .getNumber(ConfigManager::HIGHSCORES_TOP);
    ...
    {
        query <<
            "SELECT `player_skills`.`value`, `players`.`name` FROM `player_skills`,`players` WHERE `player_skills`.`skillid`="
            << skill <<
            " AND `player_skills`.`player_id`=`players`.`id` ORDER BY `player_skills`.`value` DESC, `player_skills`.`count` DESC LIMIT "
            << g_config.getNumber(ConfigManager::HIGHSCORES_TOP);
        if(!(result = db->storeQuery(query.str())))
            return hs;
...
int32_t Game::getMotdId()
{
    ...
    DBQuery query;
    query << "INSERT INTO `server_motd` (`id`, `world_id`, `text`) VALUES (" << ++lastMotdId << ", " << g_config.
        getNumber(ConfigManager::WORLD_ID) << ", " << db->escapeString(lastMotd) << ")";
...
void Game::loadMotd()
{
    const auto db = Database::getInstance();
    DBQuery query;
    query << "SELECT `id`, `text` FROM `server_motd` WHERE `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID)
...
void Game::loadPlayersRecord()
{
    const auto db = Database::getInstance();
    DBQuery query;
    query << "SELECT `record` FROM `server_record` WHERE `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID)
