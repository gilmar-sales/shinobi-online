...
bool Npc::loadFromXml(const std::string &filename)
{
    ...
    {
        std::cout << "[Notice - Npc::Npc] NPC Name: " << name << " - autowalk has been deprecated, use walkinterval." <<
            std::endl;
        walkTicks = 2000;
    }
    ...
    if(readXMLString(node, "param", strValue))
    {
        ...
        for(StringVec::iterator it = paramList.begin(); it != paramList.end(); ++it)
        {
            ...
            if(tmpParam == "male")
                ...
                std::cout << "[Warning - Npc::loadParams] NPC Name: " << name << " - Unknown param " << (*it) <<
                    std::endl;
        }
    }
            ...
            if(readXMLString(node, "file", strValue))
            {
                if(xmlDocPtr doc = xmlParseFile(getFilePath(FILE_TYPE_OTHER, "npc/lib/" + strValue).c_str()))
                {
                    ...
                    if(!xmlStrcmp(root->name, (const xmlChar *)"interaction"))
                    {
                        ...
                        std::cout << "[Error - Npc::loadInteraction] Malformed interaction file (" << strValue << ")."
                            << std::endl;
                ...
                {
                    std::cout << "[Warning - Npc::loadInteraction] Cannot load interaction file (" << strValue << ")."
                        << std::endl;
                    std::cout << getLastXMLError() << std::endl;
                }
            }
                ...
                if(it == itemListMap.end())
                {
                    ...
                    while(tmpNode)
                    {
                        if(!xmlStrcmp(tmpNode->name, (const xmlChar *)"item"))
                        {
                            ...
                            {
                                std::cout << "[Warning - Npc::loadInteraction] NPC Name: " << name <<
                                    " - Missing list item itemId" << std::endl;
                            ...
                            {
                                std::cout << "[Warning - Npc::loadInteraction] NPC Name: " << name <<
                                    " - Missing list item keywords" << std::endl;
                                tmpNode = tmpNode->next;
                                continue;
                            }
                    ...
                    std::cout << "[Warning - Npc::loadInteraction] NPC Name: " << name << " - Duplicate listId found: "
                        << strValue << std::endl;
            }
        }
                        ...
                        if(!xmlStrcmp(listNode->name, (const xmlChar *)"text"))
                        {
                            if(readXMLContentString(listNode, strValue))
                            {
                                ...
                                if(it != itemListMap.end())
                                    ...
                                    std::cout << "[Warning - Npc::loadInteraction] NPC Name: " << name <<
                                        " - Could not find a list id called: " << strValue << std::endl;
                            }
                        }
                        ...
                        if(!xmlStrcmp(subNode->name, (const xmlChar *)"action"))
                        {
                            ...
                            if(readXMLString(subNode, "name", strValue))
                            {
                                ...
                                else if(tmpStrValue == "spell")
                                {
                                    if(readXMLString(subNode, "value", strValue))
                                    {
                                        ...
                                        if(strValue != "|SPELL|" && !g_spells->getInstantSpellByName(strValue))
                                            std::cout << "[Warning - Npc::loadInteraction] NPC Name: " << name <<
                                                " - Could not find an instant spell called: " << strValue << std::endl;
                                    }
                                }
                                ...
                                else if(tmpStrValue == "teleport")
                                {
                                    ...
                                    std::cout << "[Warning - Npc::loadInteraction] Unknown action " << strValue <<
                                        std::endl;
                            }
        ...
        for(ActionList::const_iterator it = response->getFirstAction(); it != response->getEndAction(); ++it)
        {
            ...
            case ACTION_SCRIPT: {
                ...
                if(interface.reserveEnv())
                {
                    ...
                    uint32_t n = 0;
                    for(std::list<ListItem>::const_iterator iit = response->prop.itemList.begin();
        ...
        else
        {
            ...
            if(functionId != -1)
            {
                if(m_interface->reserveEnv())
                {
                    ...
                    for(ActionList::const_iterator it = response->getFirstAction(); it != response->getEndAction(); ++
                        it)
                    {
                        if(it->actionType == ACTION_SCRIPTPARAM)
                        {
                            ...
                            {
                                std::cout << "[Warning - Npc::executeResponse] Unknown script param: " << it->strValue
                                    << std::endl;
                                break;
                            }
...
void NpcScript::onCreatureAppear(const Creature *creature)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onCreatureAppear] NPC Name: " << m_npc->getName() << " - Call stack overflow"
            << std::endl;
}
...
void NpcScript::onCreatureDisappear(const Creature *creature)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onCreatureDisappear] NPC Name: " << m_npc->getName() <<
            " - Call stack overflow" << std::endl;
}
...
void NpcScript::onCreatureMove(const Creature *creature, const Position &oldPos, const Position &newPos)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onCreatureMove] NPC Name: " << m_npc->getName() << " - Call stack overflow" <<
            std::endl;
}
                              ...
                              Position *pos/* = NULL*/)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onCreatureSay] NPC Name: " << m_npc->getName() << " - Call stack overflow" <<
            std::endl;
}
                              ...
                              uint8_t count, uint8_t amount, bool ignoreCap, bool inBackpacks)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onPlayerTrade] NPC Name: " << m_npc->getName() << " - Call stack overflow" <<
            std::endl;
}
...
void NpcScript::onPlayerCloseChannel(const Player *player)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onPlayerCloseChannel] NPC Name: " << m_npc->getName() <<
            " - Call stack overflow" << std::endl;
}
...
void NpcScript::onPlayerEndTrade(const Player *player)
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onPlayerEndTrade] NPC Name: " << m_npc->getName() << " - Call stack overflow"
            << std::endl;
}
...
void NpcScript::onThink()
{
    ...
    if(m_interface->reserveEnv())
    {
        ...
        std::cout << "[Error - NpcScript::onThink] NPC Name: " << m_npc->getName() << " - Call stack overflow" <<
            std::endl;
}