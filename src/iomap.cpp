...
Tile *IOMap::createTile(Item *&ground, Item *item, uint16_t px, uint16_t py, uint16_t pz)
{
    ...
    if(ground)
    {
        if((item && item->isBlocking(nullptr)) || ground->isBlocking(nullptr))
...
bool IOMap::loadMap(const boost::shared_ptr<Map> &map, const std::string &identifier)
{
    auto start = static_cast<double>(OTSYS_TIME());

    if (!loadMapData(map, identifier))
    {
        std::cout << "> FATAL: OTBM Loader - " << getLastErrorString() << std::endl;
        return false;
    }

    std::cout << "> Map loading time: " << (static_cast<double>(OTSYS_TIME()) - start) / 1000. << " seconds." << std::endl;
    start = static_cast<double>(OTSYS_TIME());
    if (!loadSpawns(map))
    ...
    std::cout << "> Data parsing time: " << (static_cast<double>(OTSYS_TIME()) - start) / (1000.) << " seconds." << std::endl;
    start = static_cast<double>(OTSYS_TIME());

    IOMapSerialize::getInstance()->updateHouses();
    IOMapSerialize::getInstance()->updateAuctions();
    std::cout << "> Houses synchronization time: " << (static_cast<double>(OTSYS_TIME()) - start) / (1000.) << " seconds." << std::endl;

    start = static_cast<double>(OTSYS_TIME());
    IOMapSerialize::getInstance()->loadHouses();
    IOMapSerialize::getInstance()->loadMap(map);

    std::cout << "> Content unserialization time: " << (static_cast<double>(OTSYS_TIME()) - start) / (1000.) << " seconds." << std::endl;

    return true;
}

bool IOMap::loadMapData(const boost::shared_ptr<Map>& map, const std::string &identifier)
{
    FileLoader f;
            ...
            while(nodeTile != NO_NODE)
            {
                ...
                if(type == OTBM_TILE || type == OTBM_HOUSETILE)
                {
                    ...
                    if(type == OTBM_HOUSETILE)
                    {
                        ...
                        {
                            std::stringstream ss;
                            ss << "[x:" << px << ", y:" << py << ", z:" << pz << "] Could not create house id: " <<
                                _houseid;
                        ...
                        house->addTile(dynamic_cast<HouseTile *>(tile));
                    }

                    //read tile attributes
                    uint8_t tile_attribute;
                    while(propStream.GET_UCHAR(tile_attribute))
                    {
                            ...
                            if((flags & TILESTATE_REFRESH) == TILESTATE_REFRESH)
                            {
                                if(house)
                                    std::cout << "[x:" << px << ", y:" << py << ", z:" << pz <<
                                        "] House tile flagged as refreshing!";
                    ...
                    while(nodeItem)
                    {
                        if(type == OTBM_ITEM)
                        {
                            PropStream itemPropSteam;
                            f.getProps(nodeItem, itemPropSteam);

                            Item *item = Item::CreateItem(itemPropSteam);
                            if(!item)
                            {
                                std::stringstream ss;
                                ss << "[x:" << px << ", y:" << py << ", z:" << pz << "] Failed to create item.";
                            ...
                            if(item->unserializeItemNode(f, nodeItem, itemPropSteam))
                            {
                                if(house && item->isMoveable())
                                {
                            ...
                            {
                                std::stringstream ss;
                                ss << "[x:" << px << ", y:" << py << ", z:" << pz << "] Failed to load item " << item->
                                    getID() << ".";
                                setLastErrorString(ss.str());
