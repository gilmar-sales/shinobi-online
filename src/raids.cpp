...
bool Raids::parseRaidNode(xmlNodePtr raidNode, bool checkDuplicate, FileType_t pathing)
{
    ...
    {
        std::cout << "[Error - Raids::parseRaidNode] interval2 tag missing or divided by 0 for raid " << name <<
            std::endl;
        return false;
    }
    ...
    {
        file = name + ".xml";
        std::cout << "[Warning - Raids::parseRaidNode] file tag missing for raid " << name << ", using default: " <<
            file << std::endl;
    }
    else
        file = strValue;
    ...
    if (!readXMLInteger(raidNode, "margin", intValue))
        std::cout << "[Warning - Raids::parseRaidNode] margin tag missing for raid " << name << ", using default: " <<
            margin << std::endl;
    else
    ...
    if (readXMLString(raidNode, "reftype", strValue) || readXMLString(raidNode, "refType", strValue))
    {
        ...
        if (tmpStrValue == "single")
            ...
            std::cout << "[Warning - Raids::parseRaidNode] Unknown reftype \"" << strValue << "\" for raid " << name <<
                std::endl;
    }
...
bool Raid::loadFromXml(const std::string& _filename)
{
    ...
    while (eventNode)
    {
        ...
        {
            std::cout << "[Error - Raid::loadFromXml] Could not configure raid in file: " << _filename <<
...
bool AnnounceEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (readXMLString(eventNode, "type", strValue))
    {
        ...
        if (tmpStrValue == "warning")
            ...
            std::cout <<
                "[Notice - AnnounceEvent::configureRaidEvent] Unknown type tag for announce event, using default: "
...
bool EffectEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (!readXMLInteger(eventNode, "id", intValue))
    {
        ...
        {
            std::cout << "[Error - EffectEvent::configureRaidEvent] id (or name) tag missing for effect event." <<
                std::endl;
            return false;
        }
        else
...
bool ItemSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (!readXMLInteger(eventNode, "id", intValue))
    {
        if (!readXMLString(eventNode, "name", strValue))
        {
            std::cout << "[Error - ItemSpawnEvent::configureRaidEvent] id (or name) tag missing for itemspawn event." <<
                std::endl;
            return false;
        }
        else
    ...
    else
    {
        ...
        if (posList.size() < 3)
        {
            std::cout << "[Error - ItemSpawnEvent::configureRaidEvent] Malformed pos tag for itemspawn event." <<
                std::endl;
            return false;
        }
...
bool ItemSpawnEvent::executeEvent() const
{
    ...
    if (it.stackable && m_subType > 100)
    {
        ...
        while (subCount > 0)
        {
            ...
            if (!newItem)
            {
                std::cout << "[Error - ItemSpawnEvent::executeEvent] Cannot create item with id " << m_itemId <<
                    std::endl;
                return false;
            }
            ...
            if (ret != RET_NOERROR)
            {
                std::cout << "[Error - ItemSpawnEvent::executeEvent] Cannot spawn item with id " << m_itemId <<
                    std::endl;
                return false;
            }
...
bool SingleSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (!readXMLString(eventNode, "name", strValue))
    {
        std::cout << "[Error - SingleSpawnEvent::configureRaidEvent] name tag missing for singlespawn event." <<
            std::endl;
        return false;
    }
    ...
    if (!readXMLString(eventNode, "pos", strValue))
    {
        ...
        if (!readXMLInteger(eventNode, "x", intValue))
        {
            std::cout << "[Error - SingleSpawnEvent::configureRaidEvent] x tag missing for singlespawn event." <<
                std::endl;
            return false;
        }
        ...
        if (!readXMLInteger(eventNode, "y", intValue))
        {
            std::cout << "[Error - SingleSpawnEvent::configureRaidEvent] y tag missing for singlespawn event." <<
                std::endl;
            return false;
        }
        ...
        if (!readXMLInteger(eventNode, "z", intValue))
        {
            std::cout << "[Error - SingleSpawnEvent::configureRaidEvent] z tag missing for singlespawn event." <<
                std::endl;
            return false;
        }
    ...
    else
    {
        ...
        if (posList.size() < 3)
        {
            std::cout << "[Error - SingleSpawnEvent::configureRaidEvent] Malformed pos tag for singlespawn event." <<
                std::endl;
            return false;
        }
...
bool AreaSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (readXMLInteger(eventNode, "radius", intValue))
    {
        ...
        if (readXMLString(eventNode, "centerPosition", strValue) || readXMLString(eventNode, "centerpos", strValue))
        {
            ...
            {
                std::cout <<
                    "[Error - AreaSpawnEvent::configureRaidEvent] Malformed centerPosition tag for areaspawn event." <<
                    std::endl;
                return false;
            }
        ...
        else
        {
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] centerx tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] centery tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] centerz tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
    ...
    else
    {
        if (readXMLString(eventNode, "fromPosition", strValue) || readXMLString(eventNode, "frompos", strValue))
        {
            ...
            {
                std::cout <<
                    "[Error - AreaSpawnEvent::configureRaidEvent] Malformed fromPosition tag for areaspawn event." <<
                    std::endl;
                return false;
            }
        ...
        else
        {
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] fromx tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] fromy tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] fromz tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
        ...
        if (readXMLString(eventNode, "toPosition", strValue) || readXMLString(eventNode, "topos", strValue))
        {
            ...
            {
                std::cout <<
                    "[Error - AreaSpawnEvent::configureRaidEvent] Malformed toPosition tag for areaspawn event." <<
                    std::endl;
                return false;
            }
        ...
        else
        {
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] tox tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] toy tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] toz tag missing for areaspawn event." <<
                    std::endl;
                return false;
            }
    ...
    while (monsterNode)
    {
        if (!xmlStrcmp(monsterNode->name, (const xmlChar*)"monster"))
        {
            ...
            {
                std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] name tag missing for monster node." <<
                    std::endl;
                return false;
            }
            ...
            if (!min && !max)
            {
                ...
                {
                    std::cout << "[Error - AreaSpawnEvent::configureRaidEvent] amount tag missing for monster node." <<
                        std::endl;
                    return false;
                }
...
bool AreaSpawnEvent::executeEvent() const
{
    for (MonsterSpawnList::const_iterator it = m_spawnList.begin(); it != m_spawnList.end(); it++)
    {
        ...
        for (uint32_t i = 0; i < amount; ++i)
        {
            ...
            if (!monster)
            {
                std::cout << "[Error - AreaSpawnEvent::executeEvent] Cannot create monster " << spawn->name <<
                    std::endl;
                return false;
            }
            ...
            for (int32_t t = 0; t < MAXIMUM_TRIES_PER_MONSTER; ++t)
            {
                if (!g_game.placeCreature(monster, Position(random_range(m_fromPos.x, m_toPos.x),
                                                            random_range(m_fromPos.y, m_toPos.y),
                                                            random_range(m_fromPos.z, m_toPos.z))))
                    continue;
...
bool ScriptEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if (readXMLString(eventNode, "file", strValue))
    {
        ...
        {
            std::cout << "[Error - ScriptEvent::configureRaidEvent] Cannot load raid script file (" << strValue << ")."
                << std::endl;
            return false;
        }
    }
