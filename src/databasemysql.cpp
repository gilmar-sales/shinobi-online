    ...
    m_handle({}), m_attempts(0)
{
    ...
    if(mysql_get_client_version() <= 50019)
    {
        ...
        std::cout << std::endl << "> WARNING: Outdated MySQL server detected, consider upgrading to a newer version." <<
            std::endl;
    }
    ...
    if(g_config.getBool(ConfigManager::HOUSE_STORAGE))
    {
        ...
        if(DBResult *result = storeQuery("SHOW variables LIKE 'max_allowed_packet';"))
        {
            ...
            {
                std::cout << std::endl << "> WARNING: max_allowed_packet might be set too low for binary map storage."
                    << std::endl;
                std::cout <<
    ...
    if(keepAlive)
        Scheduler::getInstance().addEvent(
            createSchedulerTask((keepAlive * 1000), boost::bind(&DatabaseMySQL::keepAlive, this)));
}
...
bool DatabaseMySQL::rollback()
{
    ...
    {
        std::cout << "mysql_rollback() - MYSQL ERROR: " << mysql_error(&m_handle) << " (" << mysql_errno(&m_handle) <<
            ")" << std::endl;
        return false;
    }
...
bool DatabaseMySQL::commit()
{
    ...
    {
        std::cout << "mysql_commit() - MYSQL ERROR: " << mysql_error(&m_handle) << " (" << mysql_errno(&m_handle) << ")"
            << std::endl;
        return false;
    }
...
bool DatabaseMySQL::executeQuery(const std::string &query)
{
    ...
    if(mysql_real_query(&m_handle, query.c_str(), query.length()))
    {
        ...
        state = false;
        std::cout << "mysql_real_query(): " << query << " - MYSQL ERROR: " << mysql_error(&m_handle) << " (" << error <<
            ")" << std::endl;
    }
...
DBResult *DatabaseMySQL::storeQuery(const std::string &query)
{
    ...
    std::cout << "mysql_store_result(): " << query << " - MYSQL ERROR: " << mysql_error(&m_handle) << " (" << error <<
        ")" << std::endl;
    return NULL;
}
...
void DatabaseMySQL::keepAlive()
{
    ...
    if(delay)
    {
        ...
        Scheduler::getInstance().addEvent(
            createSchedulerTask((delay * 1000), boost::bind(&DatabaseMySQL::keepAlive, this)));
    }
}
...
bool DatabaseMySQL::connect()
{
    ...
    if(!mysql_real_connect(&m_handle, g_config.getString(ConfigManager::SQL_HOST).c_str(),
                           g_config.getString(ConfigManager::SQL_USER).c_str(),
                           g_config.getString(ConfigManager::SQL_PASS).c_str(),
                           g_config.getString(ConfigManager::SQL_DB).c_str(), g_config.getNumber(
                               ConfigManager::SQL_PORT), NULL, 0))
    {
        std::cout << "Failed connecting to database - MYSQL ERROR: " << mysql_error(&m_handle) << " (" <<
            mysql_errno(&m_handle) << ")" << std::endl;
        return false;
    }
