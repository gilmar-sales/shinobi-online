...
Account IOLoginData::loadAccount(uint32_t accountId, bool preLoad/* = false*/) const
{
    Account account;

    DBQuery query;

    query << "SELECT `id`, `name`, `password`, `premdays`, `lastday`, `key`, `warnings` FROM `accounts` WHERE `id` = "
        << accountId << " LIMIT 1";
    DBResult* result;
    if (!(result = m_db->storeQuery(query.str())))
        return account;
    ...
    query << "SELECT `name` FROM `players` WHERE `account_id` = " << accountId << " AND `world_id` = " << g_config.
...
{
    DBQuery query;
    query << "UPDATE `accounts` SET `premdays` = " << account.premiumDays << ", `warnings` = " << account.warnings <<
        ", `lastday` = " << account.lastDay << " WHERE `id` = " << account.number << m_db->getUpdateLimiter();
    return m_db->executeQuery(query.str());
}
...
bool IOLoginData::getAccountId(const std::string& name, uint32_t& number)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `accounts` WHERE `name` " << m_db->getStringComparison() << m_db->escapeString(name) <<
        " LIMIT 1";
...
{
    DBQuery query;
    query << "SELECT `group_id` FROM `accounts` WHERE `name` " << m_db->getStringComparison() << m_db->
        escapeString(accName) << " LIMIT 1";
...
bool IOLoginData::setPassword(uint32_t accountId, std::string newPassword)
{
    ...
    DBQuery query;
    query << "UPDATE `accounts` SET `password` = " << m_db->escapeString(newPassword) << " WHERE `id` = " << accountId
        << m_db->getUpdateLimiter();
    return m_db->executeQuery(query.str());
}
...
bool IOLoginData::validRecoveryKey(uint32_t accountId, std::string recoveryKey)
{
    ...
    query << m_db->getStringComparison() << m_db->escapeString(recoveryKey) << " LIMIT 1";
...
bool IOLoginData::setRecoveryKey(uint32_t accountId, std::string newRecoveryKey)
{
    ...
    DBQuery query;
    query << "UPDATE `accounts` SET `key` = " << m_db->escapeString(newRecoveryKey) << " WHERE `id` = " << accountId <<
        m_db->getUpdateLimiter();
    return m_db->executeQuery(query.str());
}

uint64_t IOLoginData::createAccount(const std::string& name, std::string password)
{
    _encrypt(password, false);


    DBQuery query;
    query << "INSERT INTO `accounts` (`name`, `password`) VALUES (" << m_db->escapeString(name) << ", " <<
        m_db->escapeString(password) << ")";
...
const Group* IOLoginData::getPlayerGroupByAccount(uint32_t accountId) const
{
    DBQuery query;
    query << "SELECT `group_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";

    DBResult* result = m_db->storeQuery(query.str());
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
    DBQuery query;
    query <<
        "SELECT `id`, `account_id`, `group_id`, `world_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, ";
    ...
    query << m_db->getStringComparison() << m_db->escapeString(name) << " AND `world_id` = ";
    ...
    player->loginPosition = Position(result->getDataInt("posx"), result->getDataInt("posy"),
                                     result->getDataInt("posz"));
    ...
    if (rankId > 0)
    {
        query.str("");
        query <<
            "SELECT `guild_ranks`.`name` AS `rank`, `guild_ranks`.`guild_id` AS `guildid`, `guild_ranks`.`level` AS `level`, `guilds`.`name` AS `guildname` FROM `guild_ranks`, `guilds` WHERE `guild_ranks`.`id` = "
            << rankId << " AND `guild_ranks`.`guild_id` = `guilds`.`id` LIMIT 1";
        if ((result = m_db->storeQuery(query.str())))
        {
    ...
    if ((result = m_db->storeQuery(query.str())))
    {
        ...
        for (auto rit = itemMap.rbegin(); rit != itemMap.rend(); ++rit)
        {
            ...
            if (pid >= 0 && pid < 100)
            {
                if (Container* c = item->getContainer())
                {
                    if (Depot* depot = c->getDepot())
                        ...
                        std::cout << "[Error - IOLoginData::loadPlayer] Cannot load depot " << pid << " for player " <<
                            name << std::endl;
                }
                else
                    std::cout << "[Error - IOLoginData::loadPlayer] Cannot load depot " << pid << " for player " << name
                        << std::endl;
            }
            else
            {
    ...
    do
    {
        ...
        if (Item* item = Item::CreateItem(result->getDataInt("itemtype"), result->getDataInt("count")))
        {
            if (!item->unserializeAttr(propStream))
                std::cout << "[Warning - IOLoginData::loadItems] Unserialize error for item with id " << item->getID()
                    << std::endl;

            itemMap.insert_or_assign(result->getDataInt("sid"), std::make_pair(item, result->getDataInt("pid")));
        }
    }
    while (result->next());
}
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
    ...
    {
        query << " WHERE `id` = " << player->getGUID() << m_db->getUpdateLimiter();
        if (!m_db->executeQuery(query.str()))
            return false;
    ...
    if (!player->isVirtual())
    {
        ...
        if (!player->isAccountManager() && nameDescription.length() > name.length())
            query << "`description` = " << m_db->escapeString(nameDescription.substr(name.length())) << ", ";
    }
    ...
    query << "`conditions` = " << m_db->escapeBlob(conditions, conditionsSize) << ", ";
    ...
    {
        query << "`guildnick` = " << m_db->escapeString(player->guildNick) << ", ";
        query << "`rank_id` = " << IOGuild::getInstance()->getRankIdByLevel(
            player->getGuildId(), player->getGuildLevel()) << ", ";
    }
    ...
    query << "`vocation` = " << tmpVoc->getId() << " WHERE `id` = " << player->getGUID() << m_db->getUpdateLimiter();
    if (!m_db->executeQuery(query.str()))
        return false;
    ...
    {
        query.str("");
        query << "UPDATE `player_skills` SET `value` = " << player->skills[i][SKILL_LEVEL] << ", `count` = " << player->
            skills[i][SKILL_TRIES] << " WHERE `player_id` = " << player->getGUID() << " AND `skillid` = " << i << m_db->
            getUpdateLimiter();
        if (!m_db->executeQuery(query.str()))
            return false;
    }
    ...
    char buffer[280];
    DBInsert query_insert(m_db);

    query_insert.setQuery("INSERT INTO `player_spells` (`player_id`, `name`) VALUES ");
    for (LearnedInstantSpellList::const_iterator it = player->learnedInstantSpellList.begin(); it != player->
         learnedInstantSpellList.end(); ++it)
    {
        sprintf(buffer, "%d, %s", player->getGUID(), m_db->escapeString(*it).c_str());
    ...
    //save depot items
    //std::stringstream ss;
    for (auto it = player->depots.begin(); it != player->depots.end(); ++it)
    {
        /*if(it->second.second)
        {
    ...
    {
        sprintf(buffer, "%u, %u, %s", player->getGUID(), cit->first, m_db->escapeString(cit->second).c_str());
        if (!query_insert.addRow(buffer))
            return false;
    }
    ...
    if (g_config.getBool(ConfigManager::INGAME_GUILD_MANAGEMENT))
    {
        ...
        for (InvitedToGuildsList::const_iterator it = player->invitedToGuildsList.begin(); it != player->
             invitedToGuildsList.end(); ++it)
        {
    ...
    query.str("");
    if (!g_config.getBool(ConfigManager::VIPLIST_PER_PLAYER))
        query << "DELETE FROM `account_viplist` WHERE `account_id` = " << player->getAccount() << " AND `world_id` = "
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert)
{
    ...
    for (ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId)
    {
        ...
        ss << player->getGUID() << ", " << it->first << ", " << runningId << ", " << item->getID()
            << ", " << (int32_t)item->getSubType() << ", " << m_db->escapeBlob(attributes, attributesSize);
...
bool IOLoginData::playerDeath(Player* player, const DeathList& dl)
{
    ...
    uint64_t deathId = m_db->getLastInsertId();
    for (DeathList::const_iterator it = dl.begin(); i < size && it != dl.end(); ++it, ++i)
    {
        ...
        std::string name;
        uint64_t killId = m_db->getLastInsertId();
...
{
    DBQuery query;
    query << "SELECT `group_id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->escapeString(name)
...
bool IOLoginData::playerExists(std::string& name, bool multiworld /*= false*/, bool checkCache /*= true*/)
{
    ...
    query << "SELECT `id`, `name` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->
...
{
    auto it = nameCacheMap.find(guid);
    if (it != nameCacheMap.end())
    {
        name = it->second;
        return true;
...
bool IOLoginData::getGuidByName(uint32_t& guid, std::string& name, bool multiworld /*= false*/)
{
    ...
    query << "SELECT `name`, `id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->
        escapeString(name) << " AND `deleted` = 0";
    if (!multiworld)
        query << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
...
{
    DBQuery query;
    query << "SELECT `id`, `name`, `group_id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->
        escapeString(name) << " AND `deleted` = 0 AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) <<
        " LIMIT 1";
...
bool IOLoginData::changeName(uint32_t guid, std::string newName, std::string oldName)
{
    ...
    query << "INSERT INTO `player_namelocks` (`player_id`, `name`, `new_name`, `date`) VALUES (" << guid << ", "
        << m_db->escapeString(oldName) << ", " << m_db->escapeString(newName) << ", " << time(nullptr) << ")";
    if (!m_db->executeQuery(query.str()))
        return false;

    query.str("");
    query << "UPDATE `players` SET `name` = " << m_db->escapeString(newName) << " WHERE `id` = " << guid << m_db->
        getUpdateLimiter();
    if (!m_db->executeQuery(query.str()))
        return false;
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex)
{
    ...
    query <<
        ...
        g_config.getNumber(ConfigManager::SPAWNPOS_X) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_Y) << ", "
        << g_config.getNumber(ConfigManager::SPAWNPOS_Z) << ", '0', " << capMax << ", " << sex <<
        ", 0, 0, 0, 0, 1, 0, '', 0, 0, 0)";
    return m_db->executeQuery(query.str());
}

DeleteCharacter_t IOLoginData::deleteCharacter(const uint32_t accountId, const std::string& characterName)
{
    ...
    DBQuery query;
    query << "SELECT `id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->
        escapeString(characterName) << " AND `account_id` = " << accountId << " AND `deleted` = 0 LIMIT 1";
    ...
    query.str("");
    query << "UPDATE `players` SET `deleted` = 1 WHERE `id` = " << id << m_db->getUpdateLimiter();
    if (!m_db->executeQuery(query.str()))
        return DELETE_INTERNAL;

    query.str("");
    query << "DELETE FROM `guild_invites` WHERE `player_id` = " << id;
    m_db->executeQuery(query.str());
...
{
    DBQuery query;
    query << "SELECT `lastip` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->escapeString(name)
        << " AND `deleted` = 0 LIMIT 1";
...
{
    DBQuery query;
    query << "SELECT `account_id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->
        escapeString(name) << " AND `deleted` = 0 LIMIT 1";
...
bool IOLoginData::getUnjustifiedDates(uint32_t guid, std::vector<time_t>& dateList, time_t _time) const
{
    DBQuery query;
...
bool IOLoginData::getDefaultTownByName(const std::string& name, uint32_t& townId) const
{
    DBQuery query;
    query << "SELECT `town_id` FROM `players` WHERE `name` " << m_db->getStringComparison() << m_db->escapeString(name)
        << " AND `deleted` = 0 LIMIT 1";
...
bool IOLoginData::updateOnlineStatus(uint32_t guid, bool login) const
{
    DBQuery query;
    ...
    query << "UPDATE `players` SET `online` = " << value << " WHERE `id` = " << guid << m_db->getUpdateLimiter();
    return m_db->executeQuery(query.str());
}
