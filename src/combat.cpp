...
Combat::~Combat()
{
    for (std::list<const Condition*>::iterator it = params.conditionList.begin(); it != params.conditionList.end(); ++
         it)
        delete (*it);
            ...
            case FORMULA_SKILL:
                {
                    ...
                    if (const Weapon* weapon = g_weapons->getWeapon(tool))
                    {
                        ...
                        if (params.useCharges && tool->hasCharges() && g_config.getBool(
                            ConfigManager::REMOVE_WEAPON_CHARGES))
                            g_game.transformItem(tool, tool->getID(),
                                                 std::max((int32_t)0, ((int32_t)tool->getCharges()) - 1));
                    }
                    else
                        max = (int32_t)maxb;
...
ReturnValue Combat::canDoCombat(const Creature* caster, const Tile* tile, bool isAggressive)
{
    ...
    {
        bool success = true;
        CreatureEventList combatAreaEvents = const_cast<Creature*>(caster)->getCreatureEvents(
        ...
        if (const auto player = caster->getPlayer(); player && player->hasFlag(PlayerFlag_IgnoreProtectionZone))
            return RET_NOERROR;
    }
...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target)
{
    ...
    const auto combatEvents = const_cast<Creature*>(attacker)->getCreatureEvents(CREATURE_EVENT_COMBAT);

    for (const auto combatEvent : combatEvents)
    {
        if (!combatEvent->executeCombat(const_cast<Creature*>(attacker), const_cast<Creature*>(target)) && success)
            success = false;
    }
    ...
    if (const Player* targetPlayer = target->getPlayer())
    {
        ...
        if (auto attackerPlayer = attacker->getPlayer(); attackerPlayer || (attacker->getMaster()
    ...
    else if (target->getMonster())
    {
        ...
        if (const Player* attackerPlayer = attacker->getPlayer(); attackerPlayer ||
            (attacker->getMaster() && (attackerPlayer = attacker->getMaster()->getPlayer())))
        {
...
ReturnValue Combat::canTargetCreature(Player* player, const Creature* target)
{
    ...
    bool deny = false;
    for (auto it = targetEvents.begin(); it != targetEvents.end(); ++it)
    {
    ...
    if (target->getPlayer() && !Combat::isInPvpZone(player, target) && player->getSkullClient(target->getPlayer()) ==
        SKULL_NONE)
    {
...
bool Combat::isProtected(Player* attacker, Player* target)
{
    ...
    if (attacker->getZone() == ZONE_PVP && target->getZone() == ZONE_PVP && g_config.getBool(
        ConfigManager::PVP_TILE_IGNORE_PROTECTION))
        return false;
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
    ...
    if (change < 0 && caster && caster->getPlayer() && target->getPlayer() && target->getPlayer()->getSkull() !=
        SKULL_BLACK)
        change = change / 2;
...
bool Combat::CombatConditionFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
    ...
    bool result = true;
    for (std::list<const Condition*>::const_iterator it = params.conditionList.begin(); it != params.conditionList.end()
         ; ++it)
    {
...
void Combat::doCombatHealth(Creature* caster, Creature* target, int32_t minChange, int32_t maxChange,
...
void Combat::doCombatMana(Creature* caster, Creature* target, int32_t minChange, int32_t maxChange,
                          const CombatParams& params)
{
    if (params.isAggressive && (caster == target || Combat::canDoCombat(caster, target) != RET_NOERROR))
        return;
...
bool CombatArea::getList(const Position& centerPos, const Position& targetPos, std::list<Tile*>& list) const
{
    ...
    for (size_t y = 0; y < rows; ++y)
    {
        for (size_t x = 0; x < cols; ++x)
        {
            ...
            {
                if (targetPos.z < MAP_MAX_LAYERS && g_game.isSightClear(targetPos, Position(tmpX, tmpY, targetPos.z),
                                                                        true))
                {
