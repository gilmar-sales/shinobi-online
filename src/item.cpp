    ...
    else if (it.weaponType != WEAPON_NONE)
    {
        ...
        if (it.weaponType == WEAPON_DIST && it.ammoType != AMMO_NONE)
        {
            ...
            if (it.attack || it.extraAttack || (item && (item->getAttack() || item->getExtraAttack())))
            {
                ...
                if (it.extraAttack || (item && item->getExtraAttack()))
                    s << " " << std::showpos << int32_t(item ? item->getExtraAttack() : it.extraAttack) <<
                        std::noshowpos;
            }
        ...
        else if (it.weaponType != WEAPON_AMMO && it.weaponType != WEAPON_WAND)
        {
            if (it.attack || it.extraAttack || (item && (item->getAttack() || item->getExtraAttack())))
            {
                ...
                {
                    s << std::max(
                        (int32_t)0, int32_t((item ? item->getAttack() : it.attack) - it.abilities.elementDamage));
                    if (it.extraAttack || (item && item->getExtraAttack()))
                        s << " " << std::showpos << int32_t(item ? item->getExtraAttack() : it.extraAttack) <<
                            std::noshowpos;
            ...
            if (it.defense || it.extraDefense || (item && (item->getDefense() || item->getExtraDefense())))
            {
                ...
                if (it.extraDefense || (item && item->getExtraDefense()))
                    s << " " << std::showpos << int32_t(item ? item->getExtraDefense() : it.extraDefense) <<
                        std::noshowpos;
            }
        }
    ...
    else if (it.armor || (item && item->getArmor()) || it.showAttributes)
    {
        ...
        if (!show)
        {
            ...
            for (int32_t i = COMBAT_FIRST; i <= COMBAT_LAST; i++)
            {
                ...
                s << getCombatName((CombatType_t)i) << " " << std::showpos << it.abilities.reflect[REFLECT_PERCENT][i]
                    << std::noshowpos << "% for " << ss;
            }
...
std::string Item::getNameDescription(const ItemType& it, const Item* item/* = NULL*/, int32_t subType/* = -1*/,
