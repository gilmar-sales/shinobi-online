...
void ConnectionManager::addAttempt(uint32_t clientIp, int32_t protocolId, bool success)
{
    ...
    if (!success || (currentTime < it->second.lastLogin + (int32_t)g_config.getNumber(ConfigManager::RETRY_TIMEOUT) /
        1000))
...
void Connection::accept()
{
    try
    {
        ...
        boost::asio::async_read(getHandle(),
                                ...
                                boost::bind(&Connection::parseHeader, shared_from_this(),
                                            boost::asio::placeholders::error));
    }
    catch (boost::system::system_error& e)
    {
...
void Connection::parseHeader(const boost::system::error_code& error)
{
    ...
    try
    {
        ...
        boost::asio::async_read(getHandle(), boost::asio::buffer(m_msg.getBodyBuffer(), size),
                                boost::bind(&Connection::parsePacket, shared_from_this(),
                                            boost::asio::placeholders::error));
    }
    catch (boost::system::system_error& e)
    {
...
void Connection::parsePacket(const boost::system::error_code& error)
{
    ...
    --m_pendingRead;
    uint32_t length = m_msg.getMessageLength() - m_msg.getReadPos() - 4, checksumReceived = m_msg.PeekU32(), checksum =
                 0;
    if (length > 0)
...
void Connection::internalSend(OutputMessage_ptr msg)
{
    ...
    try
    {
        ...
        boost::asio::async_write(getHandle(),
                                 ...
                                 boost::bind(&Connection::onWrite, shared_from_this(), msg,
                                             boost::asio::placeholders::error));
    }
    catch (boost::system::system_error& e)
    {
