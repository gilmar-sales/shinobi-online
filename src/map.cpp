...
void Map::setTile(uint16_t x, uint16_t y, uint16_t z, Tile* newTile)
{
    ...
    if (!floor->tiles[offsetX][offsetY])
    {
        ...
        std::cout << "[Error - Map::setTile] Tile already exists - pos " << offsetX << "/" << offsetY << "/" << z <<
            std::endl;
...
void Map::getSpectators(SpectatorVec& list, const Position& centerPos, bool checkforduplicate /*= false*/,
...
bool Map::checkSightLine(const Position& fromPos, const Position& toPos) const
{
    ...
    for (; x != end.x + sx; x += sx)
    {
        ...
        if (lastrz != rz || ((toPos.x != rx || toPos.y != ry || toPos.z != rz) && (fromPos.x != rx || fromPos.y != ry ||
            fromPos.z != rz)))
        {
    ...
    while (maxSearchDist != -1 || nodes.countClosedNodes() < 100)
    {
        ...
        for (uint8_t i = 0; i < 8; ++i)
        {
            ...
            if (!outOfRange && (tile = canWalkTo(creature, pos)))
            {
                ...
                else if (!(neighbourNode = nodes.createOpenNode()))
