...
bool TalkActions::registerEvent(Event* event, xmlNodePtr p, bool override)
{
    ...
    for (StringVec::iterator it = strVector.begin(); it != strVector.end(); ++it)
    {
        ...
        if (talksMap.find(*it) != talksMap.end())
        {
            if (!override)
            {
                std::cout << "[Warning - TalkAction::configureEvent] Duplicate registered talkaction with words: " << (*
                    it) << std::endl;
                continue;
            }
            else
...
bool TalkActions::onPlayerSay(Creature* creature, uint16_t channelId, const std::string& words, bool ignoreAccess)
{
    ...
    if (player && ((!ignoreAccess && std::find(exceptions.begin(), exceptions.end(), asLowerCaseString(
                                                   player->getName())) == exceptions.end() && talkAction->getAccess() >
            player->getAccess())
    ...
    if (talkAction->isScripted())
        return talkAction->executeSay(creature, cmdstring[talkAction->getFilter()],
                                      paramstring[talkAction->getFilter()], channelId);
...
bool TalkAction::configureEvent(xmlNodePtr p)
{
    ...
    if (readXMLString(p, "filter", strValue))
    {
        ...
        if (tmpStrValue == "quotation")
            ...
            std::cout << "[Warning - TalkAction::configureEvent] Unknown filter for TalkAction: " << strValue <<
    ...
    if (readXMLString(p, "case-sensitive", strValue) || readXMLString(p, "casesensitive", strValue) || readXMLString(
        p, "sensitive", strValue))
...
bool TalkAction::loadFunction(const std::string& functionName)
{
    ...
    {
        std::cout << "[Warning - TalkAction::loadFunction] Function \"" << functionName << "\" does not exist." <<
            std::endl;
        return false;
    }
...
bool TalkAction::houseSell(Creature* creature, const std::string& cmd, const std::string& param)
{
    ...
    if (!house->isGuild())
    {
        ...
        if (housesPerAccount > 0 && Houses::getInstance()->getHousesCount(tradePartner->getAccount()) >=
            housesPerAccount)
        {
            char buffer[100];
            sprintf(buffer, "Trade player has reached limit of %d house%s per account.", housesPerAccount,
                    (housesPerAccount != 1 ? "s" : ""));
            ...
            else if (Player* _player = _creature->getPlayer())
            {
                ...
                else if (action == "town" || action == "temple")
                {
                    ...
                    g_talkActions->onPlayerSay(_player, atoi(parseParams(it, tokens.end()).c_str()),
                                               parseParams(it, tokens.end()),
                                               booleanString(parseParams(it, tokens.end())));
...
bool TalkAction::addSkill(Creature* creature, const std::string& cmd, const std::string& param)
{
    ...
    else
    {
        ...
        target->addSkillAdvance(skillId, (uint32_t)(target->getVocation()->getReqSkillTries(
                                    skillId, target->getSkill(skillId,
                                                              SKILL_LEVEL) + amount) - target->getSkill(
                                    skillId, SKILL_TRIES)), false);
    }
...
bool TalkAction::ghost(Creature* creature, const std::string& cmd, const std::string& param)
{
    ...
    else if ((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_GAMEMASTER, -1, 0, false,
