    ...
    case DATABASE_ENGINE_MYSQL:
        {
            ...
            query << "SELECT `TABLE_NAME` FROM `information_schema`.`tables` WHERE `TABLE_SCHEMA` = " << db->
                escapeString(g_config.getString(ConfigManager::SQL_DB)) << " AND `DATA_FREE` > 0;";
...
bool DatabaseManager::triggerExists(std::string trigger)
{
    ...
    {
    case DATABASE_ENGINE_SQLITE:
        query << "SELECT `name` FROM `sqlite_master` WHERE `type` = 'trigger' AND `name` = " << db->
            escapeString(trigger) << ";";
        break;

    case DATABASE_ENGINE_MYSQL:
        query << "SELECT `TRIGGER_NAME` FROM `information_schema`.`triggers` WHERE `TRIGGER_SCHEMA` = " << db->
            escapeString(g_config.getString(ConfigManager::SQL_DB)) << " AND `TRIGGER_NAME` = " << db->
            escapeString(trigger) << ";";
        break;
...
bool DatabaseManager::tableExists(std::string table)
{
    ...
    case DATABASE_ENGINE_MYSQL:
        query << "SELECT `TABLE_NAME` FROM `information_schema`.`tables` WHERE `TABLE_SCHEMA` = " << db->
            escapeString(g_config.getString(ConfigManager::SQL_DB)) << " AND `TABLE_NAME` = " << db->escapeString(table)
            << ";";
        break;
    ...
    case DATABASE_ENGINE_MYSQL:
        {
            DBQuery query;
            query << "SELECT `TABLE_NAME` FROM `information_schema`.`tables` WHERE `TABLE_SCHEMA` = " << db->
                escapeString(g_config.getString(ConfigManager::SQL_DB)) << ";";
...
uint32_t DatabaseManager::updateDatabase()
{
    ...
    {
        std::cout <<
            "> WARNING: Couldn't update database - PostgreSQL support available since version 6, please use latest pgsql.sql schema."
            << std::endl;
    ...
    case 0:
        {
            ...
            if (db->getDatabaseEngine() == DATABASE_ENGINE_SQLITE)
                db->executeQuery(
                    "CREATE TABLE IF NOT EXISTS `server_config` (`config` VARCHAR(35) NOT NULL DEFAULT '', `value` INTEGER NOT NULL);");
            else
                db->executeQuery(
                    "CREATE TABLE IF NOT EXISTS `server_config` (`config` VARCHAR(35) NOT NULL DEFAULT '', `value` INT NOT NULL) ENGINE = InnoDB;");

            db->executeQuery("INSERT INTO `server_config` VALUES ('db_version', 1);");
            if (db->getDatabaseEngine() == DATABASE_ENGINE_SQLITE)
            {
                //TODO: 0.2 migration SQLite support
                std::cerr << "> SQLite migration from 0.2 support not available, please use latest database!" <<
                    std::endl;
                return 1;
            }
                    ...
                    "CREATE TABLE IF NOT EXISTS `bans2` (`id` INT UNSIGNED NOT NULL auto_increment, `type` TINYINT(1) NOT NULL COMMENT 'this field defines if its ip, account, player, or any else ban', `value` INT UNSIGNED NOT NULL COMMENT 'ip, player guid, account number', `param` INT UNSIGNED NOT NULL DEFAULT 4294967295 COMMENT 'mask', `active` TINYINT(1) NOT NULL DEFAULT TRUE, `expires` INT UNSIGNED NOT NULL, `added` INT UNSIGNED NOT NULL, `admin_id` INT UNSIGNED NOT NULL DEFAULT 0, `comment` TEXT NOT NULL, `reason` INT UNSIGNED NOT NULL DEFAULT 0, `action` INT UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY (`id`), KEY `type` (`type`, `value`)) ENGINE = InnoDB;"))
                {
                    if (DBResult* result = db->storeQuery("SELECT * FROM `bans`;"))
                    {
                        do
                        {
                            ...
                            case 1:
                                query <<
                                    "INSERT INTO `bans2` (`type`, `value`, `param`, `active`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`) VALUES (1, "
                                    << result->getDataInt("ip") << ", " << result->getDataInt("mask") << ", " << (
                                        result->getDataInt("time") <= time(NULL) ? 0 : 1) << ", " << result->
                                    getDataInt("time") << ", 0, " << result->getDataInt("banned_by") << ", " << db->
                                    escapeString(result->getDataString("comment")) << ", " << result->
                                    getDataInt("reason_id") << ", " << result->getDataInt("action_id") << ");";
                                break;

                            case 2:
                                query <<
                                    "INSERT INTO `bans2` (`type`, `value`, `active`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`) VALUES (2, "
                                    << result->getDataInt("player") << ", " << (
                                        result->getDataInt("time") <= time(NULL) ? 0 : 1) << ", 0, " << result->
                                    getDataInt("time") << ", " << result->getDataInt("banned_by") << ", " << db->
                                    escapeString(result->getDataString("comment")) << ", " << result->
                                    getDataInt("reason_id") << ", " << result->getDataInt("action_id") << ");";
                                break;
                            ...
                            case 3:
                                query <<
                                    "INSERT INTO `bans2` (`type`, `value`, `active`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`) VALUES (3, "
                                    << result->getDataInt("player") << ", " << (
                                        result->getDataInt("time") <= time(NULL) ? 0 : 1) << ", " << result->
                                    getDataInt("time") << ", 0, " << result->getDataInt("banned_by") << ", " << db->
                                    escapeString(result->getDataString("comment")) << ", " << result->
                            ...
                            case 5:
                                query <<
                                    "INSERT INTO `bans2` (`type`, `value`, `active`, `expires`, `added`, `admin_id`, `comment`, `reason`, `action`) VALUES ("
                                    << result->getDataInt("type") << ", " << result->getDataInt("player") << ", " << (
                                        result->getDataInt("time") <= time(NULL) ? 0 : 1) << ", 0, " << result->
                                    getDataInt("time") << ", " << result->getDataInt("banned_by") << ", " << db->
                                    escapeString(result->getDataString("comment")) << ", " << result->
                                    getDataInt("reason_id") << ", " << result->getDataInt("action_id") << ");";
    ...
    case 1:
        {
            ...
            if ((result = db->storeQuery(
                "SELECT `player_id`, `value` FROM `player_storage` WHERE `key` = 30018 AND `value` > 0")))
            {
                do
                {
                    query << "UPDATE `players` SET `promotion` = " << result->getDataLong("value") << " WHERE `id` = "
            ...
            if ((result = db->storeQuery("SELECT `id` FROM `accounts`;")))
            {
                do
                {
                    query << "UPDATE `accounts` SET `name` = '" << result->getDataInt("id") << "' WHERE `id` = " <<
                        result->getDataInt("id") << ";";
                    db->executeQuery(query.str());
                    query.str("");
                }
                while (result->next());
    ...
    case 2:
        {
            ...
            db->executeQuery(
                "UPDATE `players` SET `vocation` = `vocation` - 4 WHERE `vocation` >= 5 AND `vocation` <= 8;");

            DBResult* result;
            if ((result = db->storeQuery("SELECT COUNT(`id`) AS `count` FROM `players` WHERE `vocation` > 4;"))
                && result->getDataInt("count"))
            {
                std::cout << "[Warning] There are still " << result->getDataInt("count") <<
                    " players with vocation above 4, please mind to update them manually." << std::endl;
                result->free();
            }
    ...
    case 6:
        {
            ...
            {
                if (DBResult* result = db->storeQuery(
                    "SELECT `r`.`id`, `r`.`guild_id` FROM `guild_ranks` r LEFT JOIN `guilds` g ON `r`.`guild_id` = `g`.`id` WHERE `g`.`ownerid` = `g`.`id` AND `r`.`level` = 3;"))
                {
                    do
                    {
                        query <<
                            "UPDATE `guilds`, `players` SET `guilds`.`ownerid` = `players`.`id` WHERE `guilds`.`id` = "
                            << result->getDataInt("guild_id") << " AND `players`.`rank_id` = " << result->
                            getDataInt("id") << ";";
                        db->executeQuery(query.str());
                        query.str("");
                    }
    ...
    case 10:
        {
            ...
            if (tableExists("map_storage"))
            {
                ...
                db->executeQuery(
                    "ALTER TABLE `house_data` ADD `world_id` TINYINT(2) UNSIGNED NOT NULL DEFAULT 0 AFTER `house_id`;");
            }
            else if (!tableExists("house_storage"))
            {
                query <<
                    "CREATE TABLE `house_data` (`house_id` INT UNSIGNED NOT NULL, `world_id` TINYINT(2) UNSIGNED NOT NULL DEFAULT 0, `data` LONGBLOB NOT NULL, UNIQUE (`house_id`, `world_id`)";
                if (db->getDatabaseEngine() == DATABASE_ENGINE_SQLITE)
    ...
    case 11:
        {
            ...
            db->executeQuery(
                "UPDATE `players` SET `loss_experience` = `loss_experience` * 10, `loss_mana` = `loss_mana` * 10, `loss_skills` = `loss_skills` * 10, `loss_items` = `loss_items` * 10;");
            switch (db->getDatabaseEngine())
            {
            case DATABASE_ENGINE_MYSQL:
                {
    ...
    case 21:
        {
            ...
            case DATABASE_ENGINE_MYSQL:
                db->executeQuery(
                    "CREATE TABLE `account_viplist` (`account_id` INT NOT NULL, `world_id` TINYINT(2) UNSIGNED NOT NULL DEFAULT 0, `player_id` INT NOT NULL, KEY (`account_id`), KEY (`player_id`), KEY (`world_id`), UNIQUE (`account_id`, `player_id`), FOREIGN KEY (`account_id`) REFERENCES `accounts`(`id`) ON DELETE CASCADE, FOREIGN KEY (`player_id`) REFERENCES `players`(`id`) ON DELETE CASCADE) ENGINE = InnoDB;");
                break;
            case DATABASE_ENGINE_SQLITE:
                db->executeQuery(
                    "CREATE TABLE `account_viplist` (`account_id` INTEGER NOT NULL, `world_id` INTEGER NOT NULL DEFAULT 0, `player_id` INTEGER NOT NULL, UNIQUE (`account_id`, `player_id`), FOREIGN KEY `account_id` REFERENCES `accounts` (`id`), FOREIGN KEY `player_id` REFERENCES `players` (`id`));");
                break;
            default:
                break;
            }
            ...
            if (g_config.getBool(ConfigManager::ACCOUNT_MANAGER))
            {
                ...
                if (DBResult* result = db->storeQuery(query.str()))
                {
                    ...
                    do
                    {
                        ...
                        query << "UPDATE `accounts` SET `key` = " << db->escapeString(key) << " WHERE `id` = " << result
                            ->getDataInt("id") << db->getUpdateLimiter();
                        db->executeQuery(query.str());
                        query.str("");
                    }
                    while (result->next());
    ...
    if (getDatabaseConfig("encryption", value))
    {
        if (newValue != (Encryption_t)value)
        {
            ...
            case ENCRYPTION_MD5:
                {
                    ...
                    {
                        std::cout <<
                            "> WARNING: You cannot change the encryption to MD5, change it back in config.lua to \"sha1\"."
                            << std::endl;
                        return;
                    }
                    ...
                    const auto db = Database::getInstance();
                    DBQuery query;
                    if (db->getDatabaseEngine() != DATABASE_ENGINE_MYSQL && db->getDatabaseEngine() !=
                        DATABASE_ENGINE_POSTGRESQL)
                    {
                        if (DBResult* result = db->storeQuery("SELECT `id`, `password`, `key` FROM `accounts`;"))
                        {
                            do
                            {
                                query << "UPDATE `accounts` SET `password` = " << db->
                                    escapeString(transformToMD5(result->getDataString("password"), false)) <<
                                    ", `key` = " << db->escapeString(
                                        transformToMD5(result->getDataString("key"), false)) << " WHERE `id` = " <<
                                    result->getDataInt("id") << ";";
                                db->executeQuery(query.str());
                            }
                            while (result->next());
            ...
            case ENCRYPTION_SHA1:
                {
                    ...
                    {
                        std::cout <<
                            "> WARNING: You cannot change the encryption to SHA1, change it back in config.lua to \"md5\"."
                            << std::endl;
                        return;
                    }
                        ...
                        DATABASE_ENGINE_POSTGRESQL)
                    {
                        if (DBResult* result = db->storeQuery("SELECT `id`, `password`, `key` FROM `accounts`;"))
                        {
                            ...
                            {
                                query << "UPDATE `accounts` SET `password` = " << db->
                                    escapeString(transformToSHA1(result->getDataString("password"), false)) <<
                                    ", `key` = " << db->escapeString(
                                        transformToSHA1(result->getDataString("key"), false)) << " WHERE `id` = " <<
                                    result->getDataInt("id") << ";";
                                db->executeQuery(query.str());
                            }
                            while (result->next());
            ...
            default:
                {
                    std::cout <<
                        "> WARNING: You cannot switch from hashed passwords to plain text, change back the passwordType in config.lua to the passwordType you were previously using."
                        << std::endl;
                    break;
                }
            }
            ...
            case ENCRYPTION_MD5:
                {
                    const auto db = Database::getInstance();
                    DBQuery query;
                    query << "UPDATE `accounts` SET `password` = " << db->escapeString(transformToMD5("1", false)) <<
                        " WHERE `id` = 1 AND `password` = '1';";
                    db->executeQuery(query.str());
                    break;
                }
            ...
            case ENCRYPTION_SHA1:
                {
                    const auto db = Database::getInstance();
                    DBQuery query;
                    query << "UPDATE `accounts` SET `password` = " << db->escapeString(transformToSHA1("1", false)) <<
                        " WHERE `id` = 1 AND `password` = '1';";
                    db->executeQuery(query.str());
                    break;
                }
