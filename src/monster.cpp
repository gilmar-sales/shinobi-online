...
void Monster::updateIdleStatus()
{
    ...
    if (conditions.empty())
    {
        if (isSummon())
        {
            if ((!isMasterInRange && !teleportToMaster) || (getMaster()->getMonster() && getMaster()->getMonster()->
                getIdleStatus()))
                idle = true;
        }
...
bool Monster::canUseAttack(const Position& pos, const Creature* target) const
{
    ...
    for (SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
    {
        if ((*it).range != 0 && std::max(std::abs(pos.x - targetPos.x), std::abs(pos.y - targetPos.y)) <= (int32_t)(*it)
            .range)
                           ...
                           bool keepDistance /*= true*/)
{
    ...
    uint32_t tmpDist, centerToDist = std::max(std::abs(creaturePos.x - centerPos.x),
                                              std::abs(creaturePos.y - centerPos.y));
    DirVector dirVector;
    if (!keepDistance || creaturePos.y - centerPos.y >= 0)
    {
        ...
        if (tmpDist == centerToDist && canWalkTo(creaturePos, NORTH))
        {
            bool result = true;
            if (keepAttack)
                result = (!canDoAttackNow || canUseAttack(Position(creaturePos.x, creaturePos.y - 1, creaturePos.z),
                                                          attackedCreature));
    ...
    if (!keepDistance || creaturePos.x - centerPos.x >= 0)
    {
        ...
        if (tmpDist == centerToDist && canWalkTo(creaturePos, EAST))
        {
            bool result = true;
            if (keepAttack)
                result = (!canDoAttackNow || canUseAttack(Position(creaturePos.x + 1, creaturePos.y, creaturePos.z),
                                                          attackedCreature));
