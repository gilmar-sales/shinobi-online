...
function doKagemane(cid, oldPosition, oldDir, shadowsId, shadowList)
    if (not isCreature(cid)) then
...
function addKagemaneTarget(cid, target)
    ...
    if isPlayer(cid) then
        if (isPlayer(target)) then
            doPlayerSendTextMessage(target, 22,
                "You have been caught on " .. getCreatureName(cid) .. "'s kagemane no jutsu!")
        end
    end
...
function removeKagemaneTarget(cid, target)
    if not isCreature(cid) then
        if (isPlayer(target)) then
            doPlayerSendTextMessage(target, 22,
                "You have been freed from " .. getCreatureName(cid) .. "'s kagemane no jutsu!")
        else
    ...
    if (isPlayer(target)) then
        doPlayerSendTextMessage(target, 22, "You have been freed from " .. getCreatureName(cid) ..
            "'s kagemane no jutsu!")
    end
...
function addKagemaneTargetToList(cid, target)
    ...
    for i = STORAGE_KAGEMANE_TARGETS_START, STORAGE_KAGEMANE_TARGETS_END do
        ...
        if storage < 1 then
            ...
            setPlayerStorageValue(cid, STORAGE_KAGEMANE_TRAPED_QTY,
                getPlayerStorageValue(cid, STORAGE_KAGEMANE_TRAPED_QTY) + 1)
            return true
        end
    end
end
