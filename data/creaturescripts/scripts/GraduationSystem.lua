        ...
        if (thing.uid == cid) then
            ...
            if isPlayer(thing.uid) then
                if (pGrad ~= -1) then
                    table.insert(desc,
                        "You see " .. pName ..
                        " (Level " .. pLevel .. "). " .. pGender .. " is " .. pGrad .. " from clan " .. pVoc .. ".")
                else
                    table.insert(desc,
                        "You see " ..
                        pName .. " (Level " .. pLevel .. "). " .. pGender .. " is a player with error.\n(Contact a GM)")
                    return true
                end
                if (getPlayerGuildId(thing.uid) > 0) then
                    table.insert(desc,
                        " " ..
                        pGender .. " is " .. getPlayerGuildRank(thing.uid) .. " of the " .. getPlayerGuildName(thing.uid))
                    if (getPlayerGuildNick(cid) ~= "") then
