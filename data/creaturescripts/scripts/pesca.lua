function onDirection(cid, old, current)
    ...
    if peixe > 0 then
        ...
        local interval = 250
        local times = 1
        local pos = { x = getPlayerStorageValue(cid, PLAYERPOSBOIAX), y = getPlayerStorageValue(cid, PLAYERPOSBOIAY), z =
        getPlayerStorageValue(cid, PLAYERPOSBOIAZ), stackpos = 1 }
        local directionMove = getDirectionTo(pos, posPlayer)

        doMoveBoia(cid, pos, directionMove, interval, times)
        pos = { x = getPlayerStorageValue(cid, PLAYERPOSBOIAX), y = getPlayerStorageValue(cid, PLAYERPOSBOIAY), z =
        getPlayerStorageValue(cid, PLAYERPOSBOIAZ), stackpos = 1 }
