...
---Favor n�o tirar os cr�ditos, vlw.
local players = {}
...
--Esta fun��o evita que as casas salvem com intervalo de menos de 30 segundos
local function Veri2()
    verificacao2 = 0
end

---Essa fun��o evita que o sistema recome�e antes de acabar, para evitar que fique 2 ou mais sistemas rodando.
local function Veri()
    verificacao = 0
end
            ...
            if (isPlayer(tid)) then
                if getPlayerGroupId(tid) >= 6 then
                    if not exhaustion.get(players[num], storage) == false then
                        addEvent(doPlayerSendChannelMessage, 150, tid, "",
                            "Infomacoes do save para " ..
                            #getPlayersOnline() ..
                            " jogadores online. Personagem: " ..
                            getPlayerNameDescription(players[num]) ..
                            "[" .. num .. "]. Ser� salvo em: " .. exhaustion.get(players[num], storage) .. " segundos.",
                            TALKTYPE_CHANNEL_W, 10)
                    else
                        if getGameState() == GAMESTATE_NORMAL then
                            addEvent(doPlayerSendChannelMessage, 150, tid, "",
                                "Infomacoes do save para " ..
                                #getPlayersOnline() ..
                                " jogadores online. Personagem salvo agora: " ..
                                getPlayerNameDescription(players[num]) .. "[" .. num .. "].", TALKTYPE_CHANNEL_W, 10)
                        else
                            addEvent(doPlayerSendChannelMessage, 150, tid, "",
                                "Infomacoes do save para " ..
                                #getPlayersOnline() ..
                                " jogadores online. Personagem sera salvo quando sair de manutencao: " ..
                                getPlayerNameDescription(players[num]) .. "[" .. num .. "].", TALKTYPE_CHANNEL_W, 10)
                        end
                    end
                end
