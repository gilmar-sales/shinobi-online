    ...
    elseif param:lower() == "items" then
        ...
        if param[1]:lower() == "attack" then
            ...
            if exhaustion.get(cid, petStorages.exhaust) then
                return doPlayerSendCancel(cid, "You are exhausted.") and
                doSendMagicEffect(getCreaturePosition(getCreatureSummons(cid)[1]), CONST_ME_POFF)
            end
        ...
        elseif param[1]:lower() == "move" then
            ...
            if l[param[2]:lower()] then
                ...
                if ret == RETURNVALUE_NOERROR then
                    ...
                    return doPlayerSendCancel(cid, "There is not enough room.") and
                    doSendMagicEffect(getCreaturePosition(pet), CONST_ME_POFF)
                end
            else
        ...
        elseif isInArray({ "mana", "health" }, param[1]:lower()) then
            ...
            return doSendMagicEffect(getCreaturePosition(pet),
                (param[1]:lower() == "health" and CONST_ME_MAGIC_RED or CONST_ME_MAGIC_BLUE))
        end
