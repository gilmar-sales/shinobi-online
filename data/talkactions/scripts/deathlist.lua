...
function onSay(cid, words, param, channel)
    ...
    local str, deaths = "",
        db.getResult("SELECT `id`, `date`, `level` FROM `player_deaths` WHERE `player_id` = " ..
        targetId .. " ORDER BY `date` DESC LIMIT 0, " .. config.maxDeathRecords)
    if (deaths:getID() ~= -1) then
        repeat
            ...
            deaths:getDataInt("id") .. " ORDER BY killers.final_hit DESC, killers.id ASC" .. config.limit)
