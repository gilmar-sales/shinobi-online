...
function onSay(cid, words, param, channel)
    ...
    local contents, result = { day = {}, week = {}, month = {} },
        ...
        getPlayerGUID(cid) ..
        " AND `k`.`unjustified` = 1 AND `pd`.`date` >= " .. (time - (30 * 86400)) .. " ORDER BY `pd`.`date` DESC")
    ...
    if (config.advancedFragList) then
        ...
        if (skullEnd > 0) then
            result = result ..
            "\nYour " ..
            (getCreatureSkullType(cid) == SKULL_RED and "red" or "black") ..
            " skull will expire at " .. os.date("%d %B %Y %X", skullEnd)
        end
        ...
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE,
            ...
            size.day ..
            " frags today, " ..
            (size.day + size.week) .. " this week and " .. (size.day + size.week + size.month) .. " this month.")
        if (size.day > 0) then
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE,
                "Last frag at " ..
                os.date("%d %B %Y %X", contents.day[1].date) ..
                " on level " .. contents.day[1].level .. " (" .. contents.day[1].name .. ").")
        end
