...
function onSay(cid, words, param)
    if (words == "/mute") then
        ...
        if (not pid or (isPlayerGhost(pid) and getPlayerGhostAccess(pid) > getPlayerGhostAccess(cid))) then
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE,
                "Player with this name doesn\'t exist or is offline.")
            return TRUE
        end
        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voc� mutou o jogador " .. t[1] .. " por " .. time ..
        " minutos.")
        doAddCondition(player, v[tonumber(time)])
        setPlayerStorageValue(player, 90000, os.time() + time * 60)
        doPlayerSendTextMessage(player, MESSAGE_INFO_DESCR, "Voc� foi mutado por " .. time .. " minutos.")
    elseif (words == "/desmute") then
        ...
        if getCreatureCondition(player, CONDITION_MUTED) == false then
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "este jogador n�o est� mutado.")
            return true
        end
        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voc� desmutou o jogador " .. param .. ".")
        doRemoveCondition(player, CONDITION_MUTED)
        setPlayerStorageValue(player, 90000, -1)
        doPlayerSendTextMessage(player, MESSAGE_INFO_DESCR, "Voc� foi desmutado.")
    end
    return true
end
