...
--instru��es: registrar esse script no actions.xml com uid ou actionid da porta de inicio
...
local config = {
    ...
    expBase = 100,       --valor que o player ganhar� se ficar at� o final
...
local function doGiantBow(position)
    ...
    if position.x == 2576 then
        if position.y == 1712 then
            config.bowUids = { doCreateMonster("bow1", { x = position.x - 2, y = position.y, z = position.z }),
                doCreateMonster("bow2", { x = position.x - 3, y = position.y, z = position.z }) }
        else
            config.bowUids = { doCreateMonster("bow1", { x = position.x - 2, y = position.y, z = position.z }),
                doCreateMonster("bow2", { x = position.x - 3, y = position.y, z = position.z }), doCreateMonster("bow3",
                { x = position.x - 3, y = position.y - 1, z = position.z }) }
        end
    end
end
    ...
    if config.isActive then
        ...
        if currentTime < config.timeLeft then
            ...
            if isCreature(cid) then
                ...
                doSendDistanceShoot({ x = playerPos.x - 2, y = playerPos.y, z = playerPos.z },
                    { x = playerPos.x - 6, y = playerPos.y, z = playerPos.z }, CONST_ANI_BOLT)
            else
                stop()
            end
